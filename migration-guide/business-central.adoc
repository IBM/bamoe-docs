...
There will be compatability between test scenarios included in a future release of {PRODUCT}.
====

== Migration of Deprecated Assets

Though all the guided editors are deprecated in {PRODUCT} version {VERSION}, it is not a wasted effort to build these assets.

It is possible to convert certain guided assets to their non-guided and supported forms.
