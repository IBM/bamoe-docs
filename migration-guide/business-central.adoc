...
With the release of both IBM {DEV_TOOLS} and IBM {CANVAS}, most of the asset management capabilities were replaced or might take a different approach altogether.
This section describes the migration of business assets created in Business Central so that developers can maintain them by using {DEV_TOOLS} or {CANVAS} or both.
...
[NOTE]
====
At this time Test scenarios are not supported in {CANVAS} for now as it contains its own test scenario assets. There can be compatibility between the test scenarios.
====

== Migration of Deprecated Assets

While all the guided editors are deprecated in product versions, your efforts are not lost by building those assets.
...
== **Copying** asset source from Business Central
...
If you navigate over to the `Source` tab, you will quickly see that the guided rule is an editor view on top of a standard `technical rule` written using `Drools Rule Language (DLR)`, as in the following example:
...
Select the converted file and choose one of the Download buttons in Business Central, which download the converted spreadsheet file to your local machine.
