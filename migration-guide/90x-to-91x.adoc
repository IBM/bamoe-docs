...
{PRODUCT} Version {VERSION_910} follows the {PRODUCT} Version {VERSION_901}, and is optimized for a hybrid cloud environment and adapts to your domain and tooling needs.
...
Before proceeding, ensure that all prerequisite tool are installed or updated, them we need to properly setup Maven.

This migration guide will show two ways of migrating your project.
On the first way, we will be creating a new project from scracth and them moving the assets to it.
The second way, we will be updating an existing {PRODUCT_SHORT} Version {VERSION_90X} project.
Either way, the first step is to setup the Maven configuration.
...
Now the dependencies are distributed with a container image or a zip file. Please refer to the {PRODUCT_SHORT} Version {VERSION_910} Maven configuration.
// TODO: add link to the setup page.

=== Creating a new project from scracth

Now that we have Maven properly configured, we can move onto the first approach, which is to create a new project from scrath.
...
[start=7]
. Currently, the Quarkus Dev Services isn't fully supported, meaning that we need to disable it. To do that, please add the following line on the `application.properties` file:
+ 
[source]
----
...
[NOTE]
====
Please refer the https://github.com/quarkusio/quarkus/wiki/Migration-Guide-3.0[Quarkus v3 migration guide] for any other change that is related to Quarkus.
====

[start=10]
. After making all necessary changes, build the project:
+
[source,console]
----
mvn clean package
...
Upgrading a {PRODUCT_SHORT} version {VERSION_901} project to version {VERSION_910} is straigtforward.
The project sctructure didn't change, so we just need to update the project versions, some dependency names, and files.

[start=1]
. Upgrade the Quarkus version.
+
In the {PRODUCT_SHORT} Version {VERSION_901} the Quarkus version was the 2.16 and now is 3.8.4.
...
+
One of the renamed dependencies are the `groupdId=org.kie.kogito` and `artifactId=kogito-quarkus`, which was renamed to `groupdId=org.jbpm` and `jbpm-with-drools-quarkus`.
+
[source,xml]
----
       <dependency>
...
----
+
Bear in mind that it's required to make changes to any parent `pom.xml` if applied, but this will not be covered here as it depends on your the project setup.
This ends the `pom.xml` changes, and now we move to another files.

[start=4]
. Update the `application.properties` file.
+
{PRODUCT_SHORT} Version {VERSION_910} don't fully support the Quarkus Dev Services, due to that, it's required to disable this feature, and to do that, add the following line to the `application.properties` file:
+
[source]
----
 +++ quarkus.devservices.enabled=false
...
+
A major Quarkus 3 change is uses Jakarta EE 10, meaning that we also need to update any `javax` import to `jakarta`.
+
