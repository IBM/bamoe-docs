...
Now the dependencies are distributed with a container image or a zip file. Please refer to {PRODUCT_SHORT} Version {VERSION_910} xref:../installation/configure-maven.adoc[Maven configuration].
...
In the {PRODUCT_SHORT} Version {VERSION_910} some dependencies had a slight name change, so please take a look into the xref:../partials/gav-mapping-all.adoc[GAV mapping table] and update accordingly.
+
[source,xml]
----
45 +++   <dependency>
...
To do that, please add the following line in the `src/main/resources/application.properties` file:
+ 
[source]
----
...
+
{PRODUCT_SHORT} Version {VERSION_910} doesn't fully support the Quarkus Dev Services, due to that, it's required to disable this feature and to do that, add the following line to the `src/main/resources/application.properties` file:
+
[source]
----
 +++ quarkus.devservices.enabled=false
