...
The following tools are required to migrate to {PRODUCT_SHORT} version {VERSION_80x}:
...
Download the files_(replace with the most current version)_ based on your requirement.
Following are the few most common files that you can download but you select the ones that suit your environment.
...
. After you finish applying all relevant updates, start Business/Decision Central and KIE Server.
. Verify that all project data is present and accurate in Business/Decision Central, and in the top-right corner of the Business/Decision Central window, click your profile name and click About to verify the updated product version number.

+

If you encounter errors or notice any missing data in Business/Decision Central, you can restore the contents in the backup folder within the `bamoe-$VERSION-update` folder to revert the update tool changes.
