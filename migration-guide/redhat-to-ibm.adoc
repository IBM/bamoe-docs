...
The following tools are required in order to migrate to {PRODUCT_SHORT} v8:
...
_Now that you have those files, you can start to migrate our environment._
...
For a quick start see the https://maven.apache.org/guides/getting-started/[Maven Quick Start Guide].  
...
You can tailor their Maven environment to better suit their development needs by using these configurations.
...
When a bill of materials (BOM) is used it is no longer necessary to specify the dependency's version, as that comes from the BOM, so your depenencies end up looking like this:
...
So for instance, if we're running a PostgreSQL database, you must do the following:
