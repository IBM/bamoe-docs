= Stateful Workflow subsystem database tables
include::../partials/attributes.adoc[]
include::../styles/images.adoc[]

The following sections contain a description of all tables that are created by the {DDLS}  from `bamoe-{VERSION}-db-utils.zip` for stateful Workflows. These tables are also automatically created when your Business Automation application runs for the first time if the Apache KIE Flyway mechanism is enabled.
...
|Holds runtime metadata related to deadline-triggered notification settings for User Tasks

|jbpm_user_tasks_deadline_timer
|Defines the timer settings associated with a User Task deadline, used to trigger actions when the deadline is reached

|jbpm_user_tasks_reassignment
|Specifies the reassignment rules for a User Task, allowing the task to be reassigned to different users or groups under certain conditions

|jbpm_user_tasks_reassignment_timer
|Represents the timer configuration for task reassignment, enabling automated reassignment after a specified duration
|===

[#pim-db-tables]
== Process Instance Migration Add-on tables
[%autowidth]
|===
|*Table* |*Description*
|bamoe_process_instance_migration_plans  
|Stores the migration plans uploaded using the PIM Add-on

|bamoe_migration_plans_node_mappings
|Stores the migration plans node mappings

|bamoe_process_instance_migration
|Stores the state of the process instance migrations initiated with the PIM Add-on
|====
