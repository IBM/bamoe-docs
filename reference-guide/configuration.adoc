...
When DevServices are enabled, if there's no Data-Index add-on present, Kogito will spin up a supporting in-memory Data-Index Container.

[NOTE]
--
When using {PRODUCT}, using the Data Index matching the application's persistence choice (org.kie:kogito-addon-quarkus-data-index-{jpa|postgresql}) is recommended instead of using Kogito DevServices.
--
...
Firstly, the `kogito.security.auth.enabled` property must be set to true.
