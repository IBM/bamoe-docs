...
{PRODUCT} is composed of six different engines.
...
a|kogito.codegen.predictions
a|Toggle enabling code generation for the predictions engine
|boolean
a|true

a|kogito.generate.rest.predictions
a|Toggle for REST predictions endpoint generation.
|boolean
a|true
...
When using {PRODUCT}, it is recommended to use the data-index addon instead of DevServices.
...
a|kogito.devservices.image-name
...
a|kogito.devservices.service-name
...
a|jbpm.devui.users.`<user name>`.groups
...
These roles vary on the application, and no default is given.
