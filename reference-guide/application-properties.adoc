...
There are a number of different properties available in a _Business Services_ project, below is a listing of those properties, a short description, the corresponding Java type and a default, if one exists.
...
When using {PRODUCT}, it is recommended to use the Data-Index add-on instead of the in-memory Data-Index container when using DevServices.
