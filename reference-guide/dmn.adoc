...
The semantic enhancements in B-FEEL are as follows:

.Boolean logic

B-FEEL enforces a two-value logic system (true and false) by removing null from boolean results. Boolean operators such as =, <=, <, >, >=, not(), and, or, in, and between always return true or false, even when the expression involves incompatible types. In such cases, B-FEEL treats the result as false, except for !=, which returns true.
...
B-FEEL ensures string-returning functions produce an empty string ("") instead of null.
...
B-FEEL replaces null with default epoch values for date and time functions.
...
B-FEEL returns zero durations instead of null.
...
B-FEEL modifies collection-returning functions to return empty collections. The mode() function ignores non-numeric values.
...
B-FEEL returns an empty range (0..0) instead of null.
...
B-FEEL modifies addition and subtraction behavior when operand types differ. It converts the non-matching type using appropriate B-FEEL functions.

[cols="30%,70%" options="header"]
|===
|Type|Behavior
|String	|Converts non-string to string; subtraction returns ""
|Number	|Converts non-number to number
|Date/Time	|Converts non-date/time to duration
|Duration	|Converts non-duration to duration
|===
...
.Multiplication and Division Semantics

B-FEEL converts mismatched types to numbers before applying multiplication or division.
