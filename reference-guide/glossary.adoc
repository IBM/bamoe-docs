...
Compact Architecture:: is a way to enable a process to run everything in an integrated way within the same __Business Service__ deployment in {PRODUCT_SHORT}. In this architecture the communication with required and/or optional subsystems is done via internal VM communication. 

Decision Manager Open Editions (DMOE)::
A licensing option that enables users to use the {PRODUCT_SHORT} components to develop and deploy __Business Services__ that utilize DMN, DRL and spreadsheet Decision Tables. 
...
Deployment:: is considered to be a consistent functional program that provides a set of defined functionalities that enable the execution of business models through __Business Services__.
...
jBPM::
Open-source project that is part of Apache KIE, that enables the excution of stateful process applications, providing a process engine, BPMN engine...
...
A licensing option that enables users to use the {PRODUCT_SHORT} components to develop and deploy __Business Services__ that utilize stateful workflow (BPMN) as well as DMN, DRL or spreadsheet Decision Tables. 
