...
Compact Architecture:: is a way to enable a process to run everything in an integrated way within the same _Business Service_ deployment in {PRODUCT_SHORT}. In this architecture the communication with required and/or optional subsystems is done via internal VM communication. 

Decision Manager Open Editions (DMOE)::
A licensing option that enables users to use the {PRODUCT_SHORT} components to develop and deploy _Business Services_ that utilize DMN, DRL and spreadsheet Decision Tables. 

Decision Table::
A set of business rules defined in a tabular format. Each row in a Decision Table is a rule, and each column is a condition, an action, or another rule attribute. A decision table in {PRODUCT_SHORT} is developed in an Excel spreadsheet or directly in the DMN Editor. 

Deployment:: is considered to be a consistent functional program that provides a set of defined functionalities that enable the execution of business models through _Business Services_.
...
Drools::
Open-source project part of Apache KIE that provides a rule engine, and DMN engine for Java™ and the JVM platform.

Engine::
An engine is a computational entity responsible for parsing and executing related models.
There are three engines available in {PRODUCT_SHORT}: a decision engine for managing the DMN model. A rule engine for managing DRL rules and Excel decision tables. A workflow engine for managing BPMN processes.
...
Process Automation Manager Open Editions (PAMOE)::
A licensing option that enables users to use the {PRODUCT_SHORT} components to develop and deploy _Business Services_ that utilize stateful workflow (BPMN) as well as DMN, DRL or spreadsheet Decision Tables. 
