...
const Form__userLookup: React.FC<any> = (props: any) => {
	const [formApi, setFormApi] = useState<any>();
	const [userData, set__userData] = useState<string>('');
	const [userId, set__userId] = useState<number>();
	/* Utility function that fills the form with the data received from the kogito runtime */
	const setFormData = (data) => {
		if (!data) {
			return;
		}
		set__userData(data?.userData ?? '');
		set__userId(data?.userId);
	};
		...
		<div className={'pf-v5-c-form'}>
			<FormGroup
				...
				<TextInput
					...
					type={'text'}
					value={userData}
					onChange={(e, newValue) => set__userData(newValue)}
				/>
			</FormGroup>
			<FormGroup
				...
				<TextInput
					...
					step={1}
					value={userId}
					onChange={(e, newValue) => set__userId(Number(newValue))}
				/>
			</FormGroup>
		</div>
	);
};
export default Form__userLookup;
