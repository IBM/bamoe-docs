...
const Form__approvalProcess: React.FC<any> = (props: any) => {
	...
	const [amount, set__amount] = useState<number>();
	const [approved, set__approved] = useState<boolean>(false);
	const [requestor, set__requestor] = useState<string>('');
	/* Utility function that fills the form with the data received from the kogito runtime */
	const setFormData = (data) => {
		if (!data) {
			return;
		}
		set__amount(data?.amount);
		set__approved(data?.approved ?? false);
		set__requestor(data?.requestor ?? '');
	};
		...
		<div className={'pf-v5-c-form'}>
			<FormGroup
				...
				<TextInput
					...
					step={0.01}
					value={amount}
					onChange={(e, newValue) => set__amount(Number(newValue))}
				/>
			</FormGroup>
			<FormGroup fieldId='uniforms-0000-0003'>
				<Checkbox
					isChecked={approved}
					isDisabled={false}
			...
			<FormGroup
				...
				<TextInput
					...
					type={'text'}
					value={requestor}
					onChange={(e, newValue) => set__requestor(newValue)}
				/>
			</FormGroup>
		</div>
	);
