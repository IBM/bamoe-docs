...
|In {Canvas}, checking `Insecurely disable TLS certificate validation` when adding a OpenShift auth session has no effect.
|When creating a new OpenShift auth session and selecting the `Insecurely disable TLS certificate validation` option, the header to disable TLS certificate validation is not being sent to the CORS Proxy service, making connecting to OpenShift instances with self-signed certificates impossible.
