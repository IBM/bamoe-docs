...
|{VERSION_950}, {VERSION_951}, {VERSION_952}
...
|BPMN Editor: Compensation Associations rendering as Sequence Flows.
|When a BPMN diagram contains Compensation Associations — connections that link an activity to its compensation handler — the BPMN Editor incorrectly renders them as Sequence Flows. This visual misrepresentation can cause confusion when reading or editing diagrams that use compensation logic.
|{VERSION_950}, {VERSION_951}, {VERSION_952} 
...
|{VERSION_950}
a|* On {VERSION_950}, add the following to your `application.properties` file:
+
`kogito.jobs-service.numberOfWorkerThreads=1`

* Upgrade to {VERSION_951} or later.

|PIM and Runtime Persistence: SQL queries are not compatible with Oracle Database versions earlier than 23c.
|The Process Instance Migration (PIM) add-on and the Runtime Persistence add-on use Common Table Expressions (CTEs) with the `VALUES` row constructor syntax for data-isolation filtering. Oracle Database versions earlier than 23c do not support this SQL syntax, which prevents PIM and data-isolation from functioning correctly when using Oracle Database 19c or other pre-23c instances.
|{VERSION_952}
|Upgrade to Oracle Database 23c or later, which introduced support for the `VALUES` row constructor syntax.
