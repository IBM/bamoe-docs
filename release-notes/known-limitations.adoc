...
|Native compilation support is not provided.
|The current version of {PRODUCT_SHORT} does not provide native image (GraalVM) support for workflows (BPMN) but partial support for rules or decision (DMN/DROOLS) projects. If you  attempt to use native compilation with these features, you might encounter build-time failures. 
