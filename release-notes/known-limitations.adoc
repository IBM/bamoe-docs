...
|Native compilation support partially available for version 9.2.1 but not available for version 9.2.0.
|* The version 9.2.0 provides no native image (GraalVM) support for workflows (BPMN), DMN, and DRL projects. You might encounter build-time failures if you attempt to use native compilation with these features. 

* The version 9.2.1 supports the native compilation only for rules and decisions but not for workflows.
