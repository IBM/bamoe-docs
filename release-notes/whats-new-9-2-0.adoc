...
The scripts can be found in the product deliverables and support both the PostgreSQL and Microsoft SQL databases. xref:../workflow/deploying-process-services.adoc#enabling-jdbc-persistence-for-stateful -workflows[Learn more...]

=== Data consistency enforced by transactional operations
In addition to supporting the Saga pattern for compensating failures in distributed operations, {PRODUCT_SHORT} handles actions across multiple subsystems in a transacional way. This means if something fails during execution for unforeseen reasons, your Process Instances, User Tasks, and/or Jobs won't be left in invalid intermediate states. xref:../workflow/deploying-process-services.adoc#transactional-operations-on-compact-architecture[Learn more...]
...
For the first time, Spring Boot 3 can be used as a foundational framework for Decisions- and Rules-based Business Service applications. xref:../editing/springboot-dmn-drl.adoc[Learn more...]
