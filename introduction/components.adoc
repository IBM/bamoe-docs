...
1. Decisions tables in Excel: A common approach for automating simpler types of business rules based on tables written on `.xls` format.
1. Technical business rules: By using Drools Rules Language (DRL), developers rely on maximum flexibility to automate rules.
