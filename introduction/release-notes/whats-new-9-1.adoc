...
From the version {VERSION_911}, the Standalone Editors are now available enabling you to embed the BPMN and DMN editors in your application, and packages. Check the download document for package names.

Standalone Editors are JavaScript libraries containing available TypeScript types with the capability of rendering classic BPMN and DMN Editors and new DMN Editor inside an arbitrary DOM element, with full context isolation by taking advantage of the Multiplying Architecture framework.
...
* Java upgrade - Version {VERSION_911} incorporates an upgraded technology stack featuring Java 17 and Jakarta EE 10.
...
For more information, see https://www.ibm.com/support/pages/node/7151078[BAMOE 9.1.0 Download Document].