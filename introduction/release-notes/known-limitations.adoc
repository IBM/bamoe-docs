...
|BAMOE canvas cannot import files from Github using the raw URL, '/:org/:repo/refs/heads/:tree/:path*' does not work.
|Due to the Github API change, the raw URL does not work. Use the URL, '/:org/:repo/:tree/:path*' to import files from Github, see https://jsw.ibm.com/browse/DBACLD-152881[issue,window=_blank]
...
|Standalone BPMN classic editor: read-only not applied to process variable tags
|Process variable tags can be edited even if a BPMN diagram is set to read-only in the standalone classic BPMN editor.

|Standalone BPMN classic editor: incorrect reference of state-control
|Though BPMN is mentioned in the top toolbar with buttons for loading files, it refers to DMN

|Standalone DMN classic editor: read-only not applied to boxed expressions
|Boxed expressions can be edited even if a DMN diagram is set to read-only in the standalone classic DMN editor.
