...
[NOTE]
====
In some cases, VS Code may not immediately recognize the `.wid` file, causing the custom Work Item to be unavailable in the designer palette. To resolve this, run git add `customtask.wid` to ensure the file is tracked. If the issue persists, try opening the BAMOE project in a new VS Code workspace.
====
...
Work item handlers, as custom code, are created outside of {PRODUCT_SHORT}. To use the code in your custom task, the code must be deployed to the server. Work item handler projects must be Java JAR files that can be placed into a Maven repository.

In {PRODUCT_SHORT} you can deploy custom tasks using two methods:
...
ifdef::DM,PAM[]
For more information, see xref:manage-service-tasks-proc-vscode_{context}[Managing custom tasks in Visual Studio Code].
endif::[]
...
Any artifacts copied in this fashion are available to {PRODUCT_SHORT} without a server restart. Viewing the artifact in the VS Code *Artifacts* page requires clicking *Refresh*.


== Registering custom tasks

{PRODUCT} must know how to associate a custom task work item with the code executed by the work item handler. The work item definition file links the custom task with the work item handler by name and Java class. The work item handler’s Java class has to be registered as usable in {PRODUCT_SHORT}.
...
{PRODUCT_SHORT} creates a WID file by default for projects that contain at least one business process. You can create a WID file when registering a work item handler or edit the default WID file.
ifdef::DM,PAM[]
For more information about WID file locations or formatting, see xref:work-item-definitions[Work item definitions].
