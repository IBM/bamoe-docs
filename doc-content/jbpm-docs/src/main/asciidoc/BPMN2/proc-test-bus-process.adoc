...
* Helper methods: Use following methods to create a new `RuntimeManager` and `RuntimeEngine` for a given set of processes with or without using persistence. For more information about persistence, see {URL_DEVELOPING_PROCESS_SERVICES}/assembly-process-engine.html[_{PROCESS_ENGINE_DOC}_].
+
--
.`RuntimeManager` and `RuntimeEngine` Management Methods
[cols="30%,70%", options="header"]
