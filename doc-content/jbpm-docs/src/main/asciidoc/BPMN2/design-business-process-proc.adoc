...
ifdef::DM,PAM[]
For a more detailed business process example, see xref:assembly-getting-started-process-services.html[_{GETTING_STARTED_PROCESSES}_].
endif::[]

.Prerequisites
* You have created or imported an {PRODUCT} project.
ifdef::DM,PAM[]
For more information about creating projects, see xref:assembly-managing-projects.html[_{MANAGING_PROJECTS}_].
endif::[]
...
ifdef::DM,PAM[]
For more information about creating users, see xref:eap-users-create-proc.html_install-on-eap[_{INSTALLING_ON_EAP}_].
endif::[]
