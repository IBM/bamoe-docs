...
= Persisting process variables in a separate database schema in {PRODUCT_SHORT}
...
ifdef::PAM[]
{URL_MANAGING_SETTINGS}/assembly-configuring-central.html[_{CONFIGURING_CENTRAL}_].
endif::[]
ifdef::JBPM[]
<<_wb.datasources>>.
endif::[]
