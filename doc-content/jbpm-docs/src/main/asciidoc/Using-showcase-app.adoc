...
* {PRODUCT} is installed on {EAP} and configured with {KIE_SERVER}. For more information see {URL_INSTALLING_AND_CONFIGURING}/assembly-install-on-eap.html[_{INSTALLING_ON_EAP}_].
* `KieLoginModule` is configured in `standalone-full.xml`. This is required for connecting to {KIE_SERVER}. For more information about configuring {KIE_SERVER}, see {URL_INSTALLING_AND_CONFIGURING}/assembly-planning.html[_{PLANNING_INSTALL}_].
* {PRODUCT} is running and you can log in to {CENTRAL} with a user that has both `kie-server` and `user` roles. For more information about roles, see {URL_INSTALLING_AND_CONFIGURING}/assembly-planning.html[_{PLANNING_INSTALL}_].
* The *IT_Orders* sample project has been imported in {CENTRAL} and deployed to {KIE_SERVER}. For more information about case management, see {URL_GETTING_STARTED}/assembly-getting-started-case-management.html[_{GETTING_STARTED_CASES}_].
...
//* {URL_DEVELOPING_PROCESS_SERVICES}/assembly-designing-and-building-cases.html[_{DESIGNING_CASES}_]
//* {URL_GETTING_STARTED}/assembly-getting-started-case-management.html[_{GETTING_STARTED_CASES}_]
