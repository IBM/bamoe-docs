...
* `KieLoginModule` is configured in `standalone-full.xml`. This is required for connecting to {KIE_SERVER}. For more information about configuring {KIE_SERVER}, see xref:assembly-planning.html[_{PLANNING_INSTALL}_].
...
* The *IT_Orders* sample project has been imported in {CENTRAL} and deployed to {KIE_SERVER}. For more information about case management, see xref:assembly-getting-started-case-management.html[_{GETTING_STARTED_CASES}_].
