...
ifdef::PAM,DM[]
For information about developing work item handlers, see xref:assembly-custom-tasks-and-work-item-handlers.html[_{CUSTOM_TASKS_AND_HANDLERS}_].
endif::PAM,DM[]
...
ifdef::PAM,DM[]
For information about using {CENTRAL} to view execution errors and to create jobs that acknowledge the errors automatically, see xref:assembly-managing-and-monitoring-business-processes.html[_{MANAGING_PROCESSES}_].
endif::PAM,DM[]
