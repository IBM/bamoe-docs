...
a|
ifdef::DM,PAM[]
xref:assemblies/assembly-managing-projects.html[_{MANAGING_PROJECTS}_]
endif::[]
...
ifdef::DM,PAM[]
xref:assemblies/assembly-packaging-deploying.html[_{PACKAGING_DEPLOYING_PROJECT}_]
endif::[]
ifdef::DROOLS,JBPM,OP[]
...
For the operator-based deployment process: xref:assemblies/assembly-openshift-operator.html[_{DEPLOYING_OPENSHIFT_OPERATOR}_]. In the {KIE_SERVER} settings, add a {KIE_SERVER} instance and then click *Set Immutable server configuration* to configure the source Git repository for an S2I deployment.
endif::[]
|===
