[id='kie-server-system-properties-con_{context}']
...
|Integer
|`0`
|The frequency used to check for pending jobs by {PRODUCT_SHORT} engine executor. By default, the value is in seconds. If the value is 0, the executor checks only once during startup.
