...
NOTE: The update tool can only apply patches to the two most recent versions. For example, to upgrade from version 8.0.0 to 8.0.8, apply patches in the following sequence: 8.0.0 → 8.0.2 → 8.0.4 → 8.0.6 → 8.0.8. 
To ensure compatibility and stability, follow a stepwise upgrade path and avoid skipping intermediate versions, as direct upgrades to the latest fix pack may cause issues. Always verify the supported upgrade paths before initiating the patch process.

NOTE: You can deploy {PRODUCT} 8.0.x as a new installation or as a patch update from the previous version to 8.0.x.

You can use the automated update tool to apply patch updates within the {PRODUCT} 8.0. Patch updates of {PRODUCT_SHORT}, such as an update from version 8.0.x to 8.0.y, include the latest security updates and bug fixes.
