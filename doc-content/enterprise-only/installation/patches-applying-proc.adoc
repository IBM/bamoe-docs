...
+
If you encounter errors or notice any missing {CENTRAL} data, you can use the {CENTRAL} migration tool on your `.niogit` directory to prepare project data and system configurations. For more information, see xref:../assemblies/assembly-redhat-to-bamoe#niogit_directory.adoc [Using migration tool to prepare Business Central `.niogit` directory for upgrade].
+
Alternatively, you can restore the contents in the `backup` folder within the `{PRODUCT_INIT}-$VERSION-update` folder to revert the update tool changes. You can also reinstall the relevant release artifacts from your previous version of {PRODUCT}. After restoring your previous distribution, you can try again to run the update.
...
//*Prerequisites*
//
//* New version of {PRODUCT_SHORT} is installed Business Central is not running.
//+
...
//* Download `bamoe-version-add-ons.zip` and extract the file to a temporary directory.
//
//*Procedure*
//
//. In the extracted `bamoe-8.0.x-add-ons.zip` folder, extract the `bamoe-8.0.x-migration-tool.zip` sub-folder. The migration tool is in the bin directory.
//. Locate the `bin/.niogit` directory in your previous installation backup, and locate the equivalent directory for your new installation.
//. Copy the `.niogit` directory from your Business Central backup directory to the bin folder of your new installation.
//+
//The copied `.niogit` folder is the directory that the migration tool will prepare for the new installation. The original `.niogit` folder remains in your old {PRODUCT_SHORT} installation and will not be modified by the migration tool in this procedure.
//+
//
//. Delete the `.index` directory from the bin directory before migrating the `.niogit` folder. This directory is automatically created when BAMOE starts and contains index data that is incompatible between versions. The bin directory will be re-indexed automatically after migration is complete.
//
//. Note the path to the copied `.niogit` directory for the new installation. The path will be required when you run the migration tool.
//
...
//----
//+
//. In your command terminal, navigate to the temporary directory where you extracted the `bamoe-8.0.x-migration-tool` folder and run the migration tool. The `$BAMOE_NIOGIT_DIR` portion is the path to the `.niogit` directory that you previously copied to the {PRODUCT_SHORT} installation.
//
//+
//
//On Linux or UNIX-based systems:
...
//
//|Project structure migration
//|Migrates the project repository structure to the new project-oriented structure used.
//
//|System configuration directory structure migration
//|Migrates the `system.git` repository structure to the new structure used in {PRODUCT_SHORT} 8.0.x.
//
//|Forms migration
...
//+
//The `.niogit` directory structure is now compatible with Business Central in {PRODUCT_SHORT}.
//+
