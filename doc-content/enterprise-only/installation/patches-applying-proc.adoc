...
{PRODUCT_SHORT} 8.0.x can be deployed as a new installation or as a patch update from the previous version. You can use the automated update tool to apply patch updates, that include security fixes and bug corrections, to {PRODUCT_SHORT} 8.0.x. 

The update tool in a given version only supports the two most recent previous versions. Upgrading across a number of versions requires a stepwise upgrade path to ensure compatibility and stability. For example, to upgrade from 8.0.0 to 8.0.8 you must upgrade in steps: 8.0.0 → 8.0.2 → 8.0.4 → 8.0.6 → 8.0.8. Direct upgrades to the latest fix pack may cause issues. Always verify the supported upgrade paths before initiating the patch process. 
