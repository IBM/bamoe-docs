...
{PRODUCT} 8.0.x can be deployed as a new installation or as a patch update from the previous version. You can use the automated update tool to apply patch updates within the {PRODUCT} 8.0.x which include security fixes and bug corrections. The update tool supports only the two most recent versions, requiring a stepwise upgrade path (for example, upgrading from 8.0.0 to 8.0.8 must follow 8.0.0 → 8.0.2 → 8.0.4 → 8.0.6 → 8.0.8). To ensure compatibility and stability, follow a stepwise upgrade path and avoid skipping intermediate versions, as direct upgrades to the latest fix pack may cause issues. Always verify the supported upgrade paths before initiating the patch process. 
