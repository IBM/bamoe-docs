...
.Prerequisites
* The *{PRODUCT} {PRODUCT_VERSION} - {CENTRAL} Standalone* (`bamoe-8.0.x-business-central-standalone.jar`) and the *{PRODUCT} {PRODUCT_VERSION} - Add-ons* (`bamoe-8.0.x-add-ons.zip`) files have been downloaded from the {PRODUCT_DOWNLOAD_LINK}[IBM Business Automation Manager Open Editions {ENTERPRISE_VERSION} downloads] page.

.Procedure

. Extract the downloaded `bamoe-8.0.x-add-ons.zip` to a temporary directory. This archive includes the `{PRODUCT_FILE}-standalone-sample-configuration.zip` file.
. Extract the `{PRODUCT_FILE}-standalone-sample-configuration.zip` file to the directory that contains the `bamoe-8.0.x-business-central-standalone.jar` file. The `{PRODUCT_FILE}-standalone-sample-configuration.zip` file contains the following sample configuration files:
+
...
+
You can run the `bamoe-8.0.x-business-central-standalone.jar` files with the sample data provided in the configuration files or you can customize the data for your requirements.
...
----
java -jar bamoe-8.0.x-business-central-standalone.jar --cli-script=application-script.cli
----
...
[source,subs="attributes+"]
----
java -jar bamoe-8.0.x-business-central-standalone.jar --cli-script=application-script.cli -D<PROPERTY>=<VALUE> -D<PROPERTY>=<VALUE>
----
+
...
----
java -jar bamoe-8.0.x-business-central-standalone.jar \
