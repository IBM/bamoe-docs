...
--
+
. To verify that {KIE_SERVER} starts successfully, send a GET request to `http://_SERVER:PORT_/kie-server/services/rest/server/` when {KIE_SERVER} is running. For more information about running {PRODUCT} on {KIE_SERVER}, see xref:../../../eap-ba-dm-run-proc.html[_Running {PRODUCT}_].
+
