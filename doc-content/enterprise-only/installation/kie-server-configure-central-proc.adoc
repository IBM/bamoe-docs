...
[NOTE]
====
If you are deploying {KIE_SERVER} on {OPENSHIFT}, see the xref:../deploying-on-openshift.html[_{DEPLOYING_OPENSHIFT_OPERATOR}_] document for instructions about configuring it to connect to {CENTRAL}.
====
...
--
+
. To verify that {KIE_SERVER} starts successfully, send a GET request to `http://_SERVER:PORT_/kie-server/services/rest/server/` when {KIE_SERVER} is running. For more information about running {PRODUCT} on {KIE_SERVER}, see xref:../doc-content/enterprise-only/installation/eap-ba-dm-run-proc.html[_Running {PRODUCT}_].
+
