...
For example, you can create a custom class JAR to provide custom Prometheus metrics in {KIE_SERVER}. For instructions about creating the custom class, see xref:assembly-managing-and-monitoring-execution-server.html[Extending Prometheus metrics monitoring in KIE Server with custom metrics] in _{MANAGING_KIE_SERVER}_.
...
** xref:assembly-managing-and-monitoring-execution-server.html[{KIE_SERVER} capabilities and extensions] in _{MANAGING_KIE_SERVER}_.
