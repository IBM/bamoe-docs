...
. Download the sample source of the router extention from the https://github.com/jboss-container-images/rhpam-7-openshift-image/tree/main/quickstarts/router-ext[GitHub repository].
...
[source]
----
FROM registry.redhat.io/ibm-bamoe/bamoe-smartrouter-rhel9:{ENTERPRISE_VERSION_LONG}
...
* *Image*: The custom image name, for example, `{PRODUCT_INIT}-smartrouter-rhel9-custom`
