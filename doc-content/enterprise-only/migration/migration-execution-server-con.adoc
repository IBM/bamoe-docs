...
.Prerequisites
* A clean {PRODUCT} {PRODUCT_VERSION} installation exists that does not contain a `.niogit` folder. If the {PRODUCT} {PRODUCT_VERSION} installation contains a `.niogit` folder, the migration will fail. For installation options, see {URL_INSTALLING_AND_CONFIGURING}/assembly-planning.html[_{PLANNING_INSTALL}_].
