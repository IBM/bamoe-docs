...
== Enhanced dependency management for Spring Boot applications

You can now manage and override Spring Boot and Spring Framework dependency versions with improved flexibility to address security vulnerabilities and apply CVE patches. The enhanced documentation provides guidance on using BOMs (Bill of Materials) for centralized version management and verifying effective dependency versions. {URL_DEVELOPING_PROCESS_SERVICES}#managing-override-dependency-proc_business-applications[Learn more…​].
