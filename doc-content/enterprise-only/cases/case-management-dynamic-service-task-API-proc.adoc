...
.Prerequisites
* You are logged in to {CENTRAL} and an IT Orders case instance has been started using the Showcase application. For more information about using Showcase, see xref:assembly-showcase-application.html[_{SHOWCASE_APPLICATION_CASE_MANAGEMENT}_].
...
NOTE: For any custom service tasks, ensure the handler is registered in the deployment descriptor in the *Work Item Handlers* section, where the name is the same as the `nodeType` used for creating a dynamic service task. For more information about registering the deployment descriptor, see xref:assembly-custom-tasks-and-work-item-handlers.html[_{DEVELOPING_PROCESS_SERVICES}_].
