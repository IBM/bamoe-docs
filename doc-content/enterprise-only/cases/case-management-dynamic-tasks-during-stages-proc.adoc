...
.Procedure
. Start a new case using the Showcase application. For more information about using Showcase, see xref:assembly-showcase-application.html[_{SHOWCASE_APPLICATION_CASE_MANAGEMENT}_].
+
...
|===
+
The `caseStageId` is the name of the stage in the case definition where the dynamic task is to be created. This can be any dynamic or service task payload. See xref:assembly-designing-business-processes.html[_{DEVELOPING_PROCESS_SERVICES}_].
