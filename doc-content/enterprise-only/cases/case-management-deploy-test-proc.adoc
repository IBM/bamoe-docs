...
.Prerequisites
* You have a running {KIE_SERVER} instance connected to {CENTRAL}. For more information see xref:assembly-install-on-eap.html[_{INSTALLING_ON_EAP}_].
...
. Use the Case Management Showcase application to start a new case instance. For instructions about using the Showcase application, see xref:assembly-showcase-application.html[_{SHOWCASE_APPLICATION_CASE_MANAGEMENT}_].
