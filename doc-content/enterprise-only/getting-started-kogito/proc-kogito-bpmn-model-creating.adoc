...
* The *KIE Business Automation Bundle* VS Code extension is installed and enabled in your VS Code IDE. For information about enabling the VS Code extension, see {URL_GETTING_STARTED_KOGITO_MICROSERVICES}/assembly-dmn-models.html[_Installing the {PRODUCT} VS Code extension bundle_].
* You have created a project and have included any Java objects required for your {KOGITO} microservice. For information about creating a project, see {URL_GETTING_STARTED_KOGITO_MICROSERVICES}/assembly-getting-started-kogito-microservices.html[_Creating a Maven project for a {KOGITO} microservice_].
