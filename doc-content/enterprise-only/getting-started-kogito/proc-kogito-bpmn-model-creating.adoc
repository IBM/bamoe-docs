...
For more information about BPMN2 support in {PRODUCT}, see xref:assembly-designing-business-processes.html[_{DESIGNING_BUSINESS_PROCESSES}_].

.Prerequisites
* https://code.visualstudio.com/[VS Code] 1.46.0 or later is installed.
* The *KIE Business Automation Bundle* VS Code extension is installed and enabled in your VS Code IDE. For information about enabling the VS Code extension, see xref:assemblies/assembly-dmn-models.html[_Installing the {PRODUCT} VS Code extension bundle_].
* You have created a project and have included any Java objects required for your {KOGITO} microservice. For information about creating a project, see xref:assembly-getting-started-kogito-microservices.html[_Creating a Maven project for a {KOGITO} microservice_].
...
| *Process Variables*
| Add any process variables for the process. Process variables are visible within the specific process instance. Process variables are initialized at process creation and destroyed on process completion. Variable *Tags* provide greater control over the variable behavior, such as whether the variable is `required` or `internal`. For more information about variables, see xref:variables-con.html_business-processes[_{DESIGNING_BUSINESS_PROCESSES}_].
...
+
--
For more information about BPMN component properties, see xref:assembly-designing-business-processes.html[_{DESIGNING_BUSINESS_PROCESSES}_].
