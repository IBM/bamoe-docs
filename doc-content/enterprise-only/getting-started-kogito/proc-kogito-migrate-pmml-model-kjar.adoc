...
After migrating a PMML model KJAR to a {KOGITO} microservice, you need to deploy the microservice using {OPENSHIFT}. For deployment options with Openshift, see xref:assembly-deploying-kogito-microservices-on-openshift.html[_OpenShift deployment options with the RHPAM Kogito Operator_].
