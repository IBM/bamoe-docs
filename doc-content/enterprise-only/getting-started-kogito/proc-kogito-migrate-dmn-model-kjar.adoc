...
+
For the procedure about creating a Maven project, see xref:assembly-getting-started-kogito-microservices.html[_Creating a Maven project for a {KOGITO} microservice_].
+
...
+
IMPORTANT: You need to import the {KOGITO} dependency of test scenarios in the `pom.xml` file of your project and create a JUnit activator using the {KIE_SERVER} REST API. For more information, see xref:assembly-test-scenarios.html[_{TESTING_DECISION_SERVICE}_].
...
After migrating a DMN model KJAR to a {KOGITO} microservice, you need to deploy the microservice using {OPENSHIFT}. For deployment options with Openshift, see xref:assembly-deploying-kogito-microservices-on-openshift.html[_OpenShift deployment options with the RHPAM Kogito Operator_].
