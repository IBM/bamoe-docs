...
//* xref:operator-deploy-central-proc.html_openshift-operator[_{DEPLOYING_OPENSHIFT_OPERATOR}_]
//* xref:assembly-openshift-templates.html[_{DEPLOYING_OPENSHIFT_TEMPLATES}_]

====

.Prerequisites
* {CENTRAL} and {KIE_SERVER} are installed. For installation options, see xref:assembly-planning.html[_{PLANNING_INSTALL}_].
