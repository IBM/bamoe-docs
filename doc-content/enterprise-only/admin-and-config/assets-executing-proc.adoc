...
.Prerequisites
* {CENTRAL} and {KIE_SERVER} are installed and running. For installation options, see xref:assembly-planning.html[_{PLANNING_INSTALL}_].
...
ifdef::DM,PAM[]
xref:assembly-test-scenarios.html["Other methods for creating and executing DRL rules"].
endif::[]
ifdef::DROOLS,JBPM,OP[]
xref:#_builddeployutilizeandrunsection[].
endif::[]
