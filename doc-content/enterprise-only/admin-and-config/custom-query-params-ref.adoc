...
The `ExecuteSQLQueryCommand` executor command runs a custom query on the {PRODUCT} database, including the audit log tables. For the schema for the audit log tables, see {URL_DEVELOPING_PROCESS_SERVICES}/assembly-process-engine.html[_{PROCESS_ENGINE_DOC}_].
