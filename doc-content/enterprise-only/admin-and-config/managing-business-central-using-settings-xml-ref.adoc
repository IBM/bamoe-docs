...
* xref:assembly-packaging-deploying.html[_Configuring an external Maven repository for {CENTRAL} and {KIE_SERVER}_]
* xref:assembly-packaging-deploying.html[_Packaging and deploying an {PRODUCT} project in Maven_]
* xref:../admin-and-config/maven-repo-using-con.html[_Maven settings and repositories for {PRODUCT}_]
* xref:assembly-configuring-central.html[_System integration with Maven_]
