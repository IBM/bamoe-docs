...
https://www.ibm.com/products/business-automation-manager-open-editions[{PRODUCT_LONG}] ({PRODUCT_SHORT}) is a Business Automation foundational platform for developing, deploying, managing, and monitoring Enterprise-grade, cloud-native _Business Services_ in hybrid-cloud environments, such as https://www.redhat.com/en/technologies/cloud-computing/openshift[Red Hat® OpenShift®] and Kubernetes.

{PRODUCT} is based on Open-Source https://kie.apache.org[Apache KIE] technologies such as Drools, jBPM, and Kogito. There is no mapping between the open source technology versions and {PRODUCT_SHORT} versions. {PRODUCT_SHORT} provides a set of libraries, applications and tools that adapt to your Software Development Life Cycle (SDLC) and integrate with services and technologies that you are probably already using:
...
* Decisions (_DMN_): Accessible to different user profiles and based on open standards for visually authoring Decision logic.
* Rules (_DRL_): Adapted to Developer user profile and based on the battle-proof Drools engine.
...
* Stateful Workflows (_Business Process_): Usually long-running, human-driven, and dependent on timer-based scheduled events. Suitable for complex business processes requiring a lot of domain knowledge from business experts, auditability and fault tolerance.

{PRODUCT} is a developer-friendly foundational platform, meaning Workflows, Decisions, Rules, and Decision Tables are all saved in text-based formats (i.e., XML and/or plain-text), making them compatible with standard software development practices like Pull Requests, Merge Requests, and code reviews. _Business Services_ are Java applications, built on top of the Maven build system conventions. As an alternative the {CANVAS} web application empowers non-developers to collaborate in the process of creating _Business Services_. Git operations are simplified to reduce friction and enable Business Analysts or domain experts to focus on the business logic, rather than the code.
...
Even though the main focus of {PRODUCT} remains in hybrid-cloud environments, more traditional architectures are also supported by {PRODUCT} with its programmatic Java APIs that enable you to embed Decisions, Rules, and Decision Tables directly into your conventional Java applications running as standalone JARs, or web apps deployed in application servers, such as https://www.redhat.com/en/technologies/jboss-middleware/application-platform[Red Hat® JBoss® Enterprise Application Platform (JBoss EAP)], https://www.ibm.com/products/websphere-application-server[IBM WebSphere Application Server], and others.

---

In the next section you will find more detail on the different components and their integration in your Dev and Runtime environments, see xref:key-components.adoc[{PRODUCT_SHORT} Components in your Software Development Lifecycle].

---

To start building your _Business Services_ with {PRODUCT_LONG}, refer to xref:../getting-started/project-setup.adoc[Getting started → Initial Business Service project setup].
