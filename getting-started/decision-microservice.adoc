...
. To deploy BAMOE Canvas gives you the option to connect to a Kubernetes cluster or to OpenShift, which is a hybrid cloud platform offered by Red Hat. For the purpose of this example we'll use a local Kubernetes cluster, but in production you may want to use your own internal Kubernetes. To continue click in "Kubernetes" (1).
+
image::bamoe-canvas-kubernetes-login.png[Kubernetes Login]
+
[NOTE]
====
If you're not connected to Extended Services, an additional modal will open showing how to setup it in your local enviroment. Click in "Setup" (1) and follow the instructions to continue with the Kubernetes login.

image::bamoe-canvas-extended-services-setup.png[Extended Services]
====

. In the "Connect to Kubernetes" modal add your Kubernetes cluster information. If you want to create a local cluster you can click in "Configure a new local kubernetes cluster through the guided wizard" (1) and follow the instructions.
+
...
. After you've setup your credentials click in "Deploy" (1) and choose the DMN model name (2).
+
image::bamoe-canvas-deploy.png[Deploy]

. In the "Dev deployments" (1) dropdown a new depoyment will appear. It will take about 30 seconds to be ready, and a green check will appear. Click in your deployment (2).
+
image::bamoe-canvas-deployments.png[Deployments]

. A new window will appear with the equivalent form of your DMN. The deployed URL can be shared with everyone that have access to your cluster.
+
