...
1. Create Decisions and Workflows on {CANVAS} then apply an Accelerator.
2. Clone one of the Accelerators locally and start from there with {DEV_TOOLS}. 
3. (Advanced) Manually create a Java project and add dependencies to the _Business Service_ project's configuration file.

== 1. Starting on {CANVAS}
...
If you want to continue developing on {DEV_TOOLS} after starting on {CANVAS}, you can create a new Git repository in your Git provider of choice then clone it locally. Alternatively you can download your newly created _Business Sevice_ project from {CANVAS} as a .zip file and unpack it in your computer.

== 2. Starting on {DEV_TOOLS}

Currenly, {DEV_TOOLS} doesn't contain any commands to create a new _Business Service_ project automatically, but using Git in a Terminal window is easy enough, since {PRODUCT_SHORT} Accelerators are publicly hosted on IBM's GitHub at https://github.com/IBM/bamoe-canvas-quarkus-accelerator.
...
- In the `src/main/resources` folder, create or move Workflows (`.bpmn`), Decisions (`.dmn`), Rules (`.drl`), spreadsheet Decision Tables (`.xls` or `.xlsx`), or Test Scenario (`.scesim`) files.

- You _Business Service_ project is now ready to be developed.

== 3. (Advanced) Manually creating a Java project
