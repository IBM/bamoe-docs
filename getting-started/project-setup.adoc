...
* Create a new project using the {CANVAS} accelerator functionality (see xref:../tools/applying-accelerators[Using Accelerators]).
...
== Source Control
Canvas only works with Git.
Any git based tools should continue to work with BAMOE projects (i.e. VS Code, Git cli, GitHub Desktop, etc.).
Add the code to Git using your preferred tool, and push to a remote.