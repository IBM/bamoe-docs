...
[NOTE]
====
Spring Boot is supported for Decisions, Rules, and spreadsheet Decision Tables services, but there's no Accelerator available for it yet. You can, however, get started with Spring Boot based on {BAMOE_DOCS_EXAMPLES_DOCS_LINK}/spring-boot/decisions/pom.xml[this sample pom.xml file for Decisions.] or {BAMOE_DOCS_EXAMPLES_DOCS_LINK}/spring-boot/rules/pom.xml[this sample pom.xml for Rules].
====
