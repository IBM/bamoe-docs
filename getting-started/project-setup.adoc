...
[NOTE]
====
Spring Boot is supported for Decisions, Rules, and spreadsheet Decision Tables services, but there's no Accelerator available for it yet. You can, however, get started with Spring Boot based on {BAMOE_DOCS_EXAMPLES_DOCS_LINK}/spring-boot/pom.xml[this sample pom.xml file.]
====
