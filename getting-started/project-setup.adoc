= Initial Business Service project setup and walkthrough
include::../partials/attributes.adoc[]

//tag::maven-setup[]
== Requirements
Set up your Maven environment, following these instructions for doing so xref:../installation/configure-maven.adoc[Installing → BAMOE Maven repository].
...
== Manually creating a Business Service project for with {PRODUCT_SHORT}
{PRODUCT_SHORT} uses a Maven project as a basis for the development of business automation artifacts. A Maven project can be created either using plugins or archetypes, for example using the following command:
...
You can then update the `pom.xml` for the project with content from one of the following templates:
...
xref:../partials/process-sample-pom.adoc[Sample Workflow Application POM Template] a sample POM that describes a typical Workflow project dependencies and plugins.
...
.. and others.
. Install {DEV_TOOLS_VSCODE_MARKETPLACE_LINK}[*{DEV_TOOLS}*].
This gives you access to the visual editors for BPMN and DMN files.
