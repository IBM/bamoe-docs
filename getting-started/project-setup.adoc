...
One last thing we're going to do is to create a GitHub repository for this service, so we have somewhere to store our changes and also take advantage of building it into a cloud service running on OpenShift. 

. You can create a repository on https://github.com[GitHub]. 
  To do this login to your GitHub account (or create one if you don't have one!) and from your home page  click the green `New` icon near the top left of the page (or you can navigate directly to https://github.com/new[here])
+
