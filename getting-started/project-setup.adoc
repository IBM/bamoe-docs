...
[NOTE]
====
Spring Boot is supported for Decisions, Rules, and spreadsheet Decision Tables services, however there is no Accelerator yet available for this framework. To get started with Spring Boot you can use https://github.com/IBM/bamoe-docs/tree/9.2.x/_code/docs/spring-boot/decisions/pom.xml[this sample pom.xml for Decisions.] or https://github.com/IBM/bamoe-docs/blob/9.2.x/_code/docs/spring-boot/rules/pom.xml[this sample pom.xml for Rules].
====

There are three options to use Accelerators to set up your project:

* a. Create your Decisions and Workflows on {CANVAS} then apply an Accelerator.
* b. Clone one of the Accelerators locally then create your Decisions and Workflows using {DEV_TOOLS}. 
* c. (Advanced) Create a Java project manually, adding dependencies to the _Business Service_ project configuration file.
...
- Your _Business Service_ project is now ready to be developed!
