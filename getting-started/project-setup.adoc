...
There are currently three available Accelerators to choose from:

- **`Quarkus (DMN)`** - For _Business Services_ that will contain Decisions, Rules, or spreadsheet Decision Tables.
- **`Quarkus (Full)`** - For _Business Services_ that will contain stateful Workflows in addition to Decisions, Rules, or spreadsheet Decision Tables.
- **`Spring Boot (DMN)`** - For _Business Services_ that will contain Decisions, Rules, or spreadsheet Decision Tables.
...
* b. Open a new or existing folder on VS Code with {DEV_TOOLS} installed, and apply an Accelerator.
...
[NOTE]
====
Configuring {CANVAS} and {DEV_TOOLS} with custom Accelerators is possible and encouraged, so new Business Services created in your organization follow standards and best practices crafted especially for your use-cases. Discover more on how to configure custom Accelerators on {CANVAS} and {DEV_TOOLS} at xref:../installation/canvas.adoc[Installing → {CANVAS}] and xref:../installation/developer-tools-for-vscode.adoc[Installing → {DEV_TOOLS}]
====
...
Since BAMOE 9.2.1, {DEV_TOOLS} provides commands to create a new _Business Service_ project more easily using Accelerators. The following steps will get you started:
...
- Select which Accelerator you want to apply.

Your _Business Service_ project on {PRODUCT_SHORT}, based on your Accelerator of choice will be ready for you in the currently open folder and a new Git repository will be initiated for you if the currently open folder isn't one yet. Existing Business Automation assets will be moved to the correct locations, and existing modifications in your staging area will be kept, so you remain in control of your development flow.
