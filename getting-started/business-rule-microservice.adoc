= Business rules microservice in 15 minutes
...
The business Decision contains the decision logic of the {PRODUCT} service. You can define business rules and decisions in different ways, such as with DMN Decisions or DRL Rules. The example for this procedure uses a DMN model.

. In the Maven project that was created in the xref:../getting-started/project-setup.adoc[Initial _Business Service_ project setup and walkthrough] section, navigate to the `src/main/java/com/ibm` folder and add the following `Person.java` file:
+
--
