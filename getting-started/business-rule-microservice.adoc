...
. In the Maven project that you generated for your {PRODUCT} service, navigate to the `src/main/java/com/ibm` folder and add the following `Person.java` file:
+
--
...
In VS Code, you can add the {VS_CODE_BUNDLE_LINK}[*{VS_CODE_BUNDLE}*] to design the decision requirements diagram (DRD), boxed expression, and data types with the DMN modeler.

To create this DMN model example in VSCode using the DMN modeler, follow these steps:

.. Create a `PersonDecisions.dmn` file, after the editor finish to load, click the *Properties* icon in the upper-right corner of the DMN modeler, and confirm that the DMN model *Name* is set to `PersonDecisions`.
