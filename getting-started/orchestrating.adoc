...
A number of samples, or example applications are shipped with IBM BAMOE in the file `bamoe-9.0.0-kogito-examples.zip`:

* `process-business-rules-quarkus`: A quickstart project that shows the use of business rules and processes
* `process-decisions-rest-quarkus`: An example project that shows the usage of decisions within processes. The focus here is to show how to integrate decisions in a remote way using REST APIs where they can be deployed decoupled from the process service.
* `process-decisions-rules-quarkus`: An example project that shows the usage of decisions within processes. The focus here is to show how to integrate decisions in an embedded way using Business Rule Task where they must be deployed together with the process.
* `process-error-handling`: A simple usage scenario of the Error Handling Strategy

For more {PRODUCT} examples that use decisions orchestrated by processes, see `bamoe-9.0.0-kogito-examples.zip`.