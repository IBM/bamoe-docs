...
The following example is a Workflow scenario in {PRODUCT} that demonstrates how you can use Workflows to orchestrate your {PRODUCT} Business Decision Services in Straight-Through Processing (STP). The example of a traffic violation project describes how to use Decisions within Workflows and how to integrate Decisions in an embedded way using Business Rule Tasks, which must be deployed with a Workflow in the same application. For more {PRODUCT} examples that use Workflow capabilities, see the references below. For more information on stateful Workflows see (xref:../workflow/overview.adoc[Overview of Stateful Workflows with Compact Architecture]).
