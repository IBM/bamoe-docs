...
+
To install the {PRODUCT} GitHub Chrome extension without the {PRODUCT} Business Modeler Hub, you can download and extract the `chrome_extension_kogito_kie_editors___VERSION__.zip` file from the https://github.com/kiegroup/kie-tools/releases[`kogito-tooling`] releases page in GitHub, and then in the upper-right corner in Chrome, go to *Customize and control* -> *Settings* -> *Extensions* -> *Load unpacked* and open the extracted `dist` folder.

* *Business Modeler online viewer*: Enables you to view and design BPMN and DMN models online at `https://sandbox.kie.org/`. The online modeler supports the `.new` format, so you can also enter `bpmn.new` or `dmn.new` in a web browser to start designing a new BPMN or DMN model in the online modeler.
* *Business Modeler standalone editors*: Enable you to view and design BPMN and DMN models embedded in your web applications. To download the necessary files, you can either use the NPM artifacts from the https://www.npmjs.com/package/@kie-tools/kie-editors-standalone[Kie Tools repository] or download the JavaScript files directly for the DMN standalone editor library at `https://sandbox.kie.org/standalone/dmn/index.js` and for the BPMN standalone editor library at `https://sandbox.kie.org/standalone/bpmn/index.js`.
...
[source]
----
<script src="https://sandbox.kie.org/standalone/dmn/index.js"></script>
<script src="https://sandbox.kie.org/standalone/bpmn/index.js"></script>
----

*Download each JavaScript file manually*: For this method, follow these steps:

.. Download the JavaScript files:
** For the DMN editor, go to https://sandbox.kie.org/standalone/dmn/index.js.
** For the BPMN editor, go to https://sandbox.kie.org/standalone/bpmn/index.js.
