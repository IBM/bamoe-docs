...
* *{CANVAS}*: (Recommended) Enables you to work with Decisions and Workflows, directly on the browser. See the xref:../tools/canvas.html[Canvas] chapter for further details.
...
[NOTE]
====
If you build a native image, add the following property to `application.properties` for the ILMT library to work properly:

[source]
----
quarkus.native.resources.includes=**swidtag
----
====
...
* https://github.com/kiegroup/kogito-runtimes/blob/main/api/kogito-events-api/src/main/java/org/kie/kogito/event/process/ProcessInstanceEventBody.java[ProcessInstanceEvent]: This event is published when a process instance is created, modified, or completed.
// * https://github.com/kiegroup/kogito-runtimes/blob/main/api/kogito-events-api/src/main/java/org/kie/kogito/event/process/UserTaskInstanceEventBody.java[UserTaskInstanceEvent]: This event is published when a change occurs on a user task.
* https://github.com/kiegroup/kogito-runtimes/blob/main/api/kogito-events-api/src/main/java/org/kie/kogito/event/process/VariableInstanceEventBody.java[VariableInstanceEvent]: This event is published when a variable is created, modified, or removed.
