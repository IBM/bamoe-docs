...
The `kmodule.xml` allows to define and configure one or more `KieBase` s and for each `KieBase` all the different `KieSession` s that can be created from it, as showed by the https://docs.drools.org/{DROOLS_VERSION}/drools-docs/drools/KIE/index.html#_thekmodulexmlfile[example].
...
=== Stateful
The first session https://docs.drools.org/{DROOLS_VERSION}/drools-docs/drools/rule-engine/index.html#kie-sessions-stateful-con_rule-engine[stateful].
...
=== Stateless
The session is https://docs.drools.org/{DROOLS_VERSION}/drools-docs/drools/rule-engine/index.html#kie-sessions-stateless-con_rule-engine[stateless].
