...
Note that for JSON marshaling, {PRODUCT_SHORT} is relying on the default mappers of https://quarkus.io/version/3.15/guides/rest-json#jackson[Quarkus], which can be configured according to the specific requirements.
...
[role="_abstract"]
For each DMN model in a _Business Service_ project, the following REST endpoints are automatically generated based on the content of the model:
