...
* link:https://kiegroup.github.io/dmn-feel-handbook/[DMN FEEL Handbook]
...
* link:#con-dmn-decision-tables[Decision tables]
* link:#con-dmn-literal-expressions[Literal expressions]
* link:#con-dmn-context-expressions[Contexts]
* link:#con-dmn-relation-expressions[Relations]
* link:#con-dmn-function-expressions[Functions]
* link:#con-dmn-invocation-expressions[Invocations]
* link:#con-dmn-list-expressions[Lists]
...
// replace with https://raw.githubusercontent.com/kiegroup/drools/8.39.0.Final/kie-dmn/kie-dmn-core/src/test/resources/org/kie/dmn/core/0019-flight-rebooking.dmn ?
...
<dmn:definitions xmlns="https://www.drools.org/kie-dmn/Flight-rebooking" xmlns:dmn="http://www.omg.org/spec/DMN/20151101/dmn.xsd" xmlns:feel="http://www.omg.org/spec/FEEL/20140401" id="_0019_flight_rebooking" name="0019-flight-rebooking" namespace="https://www.drools.org/kie-dmn/Flight-rebooking">
  ...
  <dmn:decision name="Rebooked Passengers" id="d_RebookedPassengers">
    ...
    <dmn:invocation>
      ...
      <dmn:binding>
        <dmn:parameter name="Waiting List" typeRef="tPassengerTable"/>
        <dmn:literalExpression>
          <dmn:text>Prioritized Waiting List</dmn:text>
        </dmn:literalExpression>
      </dmn:binding>
      <dmn:binding>
        <dmn:parameter name="Reassigned Passengers List" typeRef="tPassengerTable"/>
        <dmn:literalExpression>
          <dmn:text>[]</dmn:text>
        </dmn:literalExpression>
      </dmn:binding>
      <dmn:binding>
        <dmn:parameter name="Flights" typeRef="tFlightTable"/>
        <dmn:literalExpression>
...
The following DRD link:#_dmn_decision_requirements_diagram_drd_components[components] are available.
...
+
--
// here we finished some 'partial' steps how some diagram is designed
// how to add conections
// how to fill in some of expression types
// in the end we provide some picture, but no source or link to source of complete diagram
The following is an example DRD for a loan prequalification decision model:

.Completed DRD for loan prequalification
image::dmn-example-drd.png[]

// here we provide an example of a decision service node
// but in the procedure above, we do not guide any step that would lead to a decision service in the picture below
// placement of this picutre seems as 'random'
...
// documenting that user can change order of enums doesn't seems to me as bringign much value
// taking into account we use a picture for it, it prolongs the documentation, we repeat it twice in the docuemntation
...
image::dmn-custom-data-type-constraints-drag.png[]
...
--
If the DRD does not open in the {PRODUCT} DMN modeler, ensure that you have installed and enabled the {VS_CODE_BUNDLE_LINK}[*{VS_CODE_BUNDLE}*].
// extract this snippet in a seaprate .adoc and reuse, it is repeated 6x
...
// doesn't bring much value from my point of view and prolongs the documentaiton
// furthermore the screenshot is example of maintanance costs example, as is outdated
...
image::dmn-manage-data-types-structured2-drag.png[]
--
...
NOTE: Please notice {PRODUCT} doesn't contain a PMML modeler. You need to create the PMML model before it is included into DMN model using some other PMML modeler. For example the KIE community https://marketplace.visualstudio.com/items?itemName=kie-group.pmml-vscode-extension[PMML Scorecard Modeler].
