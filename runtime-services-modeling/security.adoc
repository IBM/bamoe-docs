...
After configuring your {PRODUCT_SHORT} runtimes  correctly to integrate its framework security layer, it is possible to rely on an external Identity Providers (IdPs) like Keycloak or Microsoft Entra ID as your project’s Identity and Access Management Solution. A key role of this architecture is the {MANAGEMENT_CONSOLE}, which provides an integrated environment to authenticate against your live _Business Services_ and interact with its entities like Process Instances, submit User Task forms, and view scheduled Jobs, in a secured. For more information on {MANAGEMENT_CONSOLE}, see xref:../installation/management-console.adoc[Installing → {MANAGEMENT_CONSOLE}].
