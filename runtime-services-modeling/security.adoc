...
_Business Services_ on {PRODUCT_SHORT} rely on Quakus or Spring Boot for configuring authentication and authorization, and will query their provided security-related data for information about who's the authenticated user performing a request and what permissions (roles/groups) they have, especially the stateful Workflows subsystem for User Tasks.
...
{MANAGEMENT_CONSOLE} can connect to secured _Business Services_ provided they expose enough information about their configured Identity Provider. See xref:../runtime-services-modeling/security.adoc#mgmt-console-oidc-proxy[Allowing {MANAGEMENT_CONSOLE} to connect to live _Business Services_] for more details.
