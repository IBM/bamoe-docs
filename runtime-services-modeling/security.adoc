...
For the BAMOE Management Console to authenticate and connect to a Business Service instance that is secured through an Identity Provider, the instance must make the Identity Provider URL available.

To solve this, you need to use the `quarkus-oidc-proxy` extension, as it creates new endpoints in the Business Service instance where one of them is `/q/oidc/.well-known/openid-configuration`, which provides the Identity Provider URL (the same URL that is defined in the `quarkus.oidc.auth-server-url` property).
