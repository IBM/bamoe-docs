...
- **Scope**: Overrides the scopes requested to the Identity Provider. Useful from some Identity Providers that, for example, will only grant a Refresh Token if the `offline_access` scope is included in the token request. Defaults to the value of the `RUNTIME_TOOLS_MANAGEMENT_CONSOLE_OIDC_CLIENT_DEFAULT_SCOPES` environment variable.
...
// adding the pre-configured runtimes here but it needs more thought on how to integrate with above

=== Pre-configured connected Business Services

It is possible set up a pre-configured list of Business Services, defined via the `RUNTIME_TOOLS_MANAGEMENT_CONSOLE_MANAGED_BUSINESS_SERVICES` environment variable. This allow users to connect to pre-defined listed Business Services without having to manually configure each one. To learn how to set them up check the xref:../installation/management-console.adoc##pre-configured-business-services[installation guide].

On the first load, the Management Console will create AuthSessions for each Business Service listed in the environment variable.

If a Business Service requires authentication, the Management Console display an "Authenticate" button, which will redirect to the login page for the Identity Provider configured in the Business Service, unless the user is already authenticated with the same Identity Provider (using the same `clientId`, `scopes`, and `audience`), in this case the same token will be used for all matching Business Services.
...
The Process Definitions list page shows the list of process definitions available. They are displayed in an expandable list, which shows all the child process instances of the particular process definition when expanded.
