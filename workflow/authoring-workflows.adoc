...
[#new-bpmn-editor]
== Authoring Workflows with BPMN Editor (Tech Preview)

Since version 9.2.1, {PRODUCT_SHORT} includes a new BPMN Editor as Technology Preview, bringing faster startup times and retro-compatibility with BPMN Editor (classic). The new BPMN Editor (Tech Preview) is avaialble on {CANVAS}, {DEV_TOOLS}, and as a standalone JavaScript library.

.Empty Workflow on BPMN Editor (Tech Preview)
image::workflow/new-bpmn-editor.png[BPMN Editor (Tech Preview)]

=== Enchancements

Some features from BPMN Editor (classic) have been improved on BPMN Editor (Tech Preview):
...
- Grid snapping is now available, just like on DMN Editor;
...
Some features from BPMN Editor (classic) are not yet available on BPMN Editor (Tech Preview):
