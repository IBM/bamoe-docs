...
. *Variables* → to edit process variables in a dedicated panel.
...
. *Node palette* → to select and drag node types into your workflow (variations can be selected after dragging a node type onto the Canvas).
. *Data Object* and *Keyboard Shortcuts* → to drag and create a new data object and you can view the keyboard shortcuts by clicking the keyboard button. 
. *Properties* → to manage the properties of the currently selected workflow element.
. The *Zoom* *Fit view* and *toggle interactivity* buttons enable you to manage the BPMN diagram. 
...
In the “Properties” panel under the section Implementation you can choose between DRL or DMN. Select the file you want and the Decision inside it.
Here is an example:

.Properties panel for a Business Rule Task inside a Workflow
image::workflow/decision-in-workflow.png[width=360]
...
== Configuring a Custom Work Item Handler
