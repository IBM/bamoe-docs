...
An example application `flexible-process-quarkus` can be found in the .xref:..getting-started/examples.adoc[Example __Business Service__ projects]. This example shows the use of flexible processes to implement a case management scneario using ad-hoc subprocesses, milestones, and DMN business rule integration. 
