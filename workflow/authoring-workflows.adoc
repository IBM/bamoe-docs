...
image::workflow/adhoc-proc-props.png[Ad-hoc process property settings]
...
image::workflow/adhoc-sub-proc.png[Ad-hoc subprocess]
...
image::workflow/adhoc-auto-st.png[Ad-hoc auto-start property settings]
...
image::workflow/adhoc-milestone.png[Ad-hoc milestone]
...
To implement flexible, non-linear workflows for unpredictable, one-off business cases, you can use the following core behaviors of flexible process execution: 

* Ability to trigger/re-trigger nodes using signals
* Re-trigger tasks using signals
* Ad-hoc processes
* Ad-hoc tasks
+
Dynamic ad-hoc task creation: ability to create a new task that was not initially on the process definition. Example: During onboarding or financial crime review they want to ask something for the client, so they create an ad-hoc task that gets assigned to the client.
+
* Milestones
** Rest endpoints
** Ability to trigger by throwing signal event
** Depends on the ability of the engine to manage signal scopes, as you can set the scope to restrict a signal to only trigger milestones in the scope of that process instance.
* BPMN
** ad-hoc processes, tasks are on the process, and can be executed in any order
** They use their own case id, the wrapper maps the {PRODUCT_SHORT} `caseid` to their custom id