...
NOTE: If your {BAMOE} application uses Microsoft SQL Server persistence in Quarkus Dev Mode and want to use the DataBase Dev Services,
...
To initialize your application database, {BAMOE} provides a set of DDL (Data Definition Language) scripts in the `bamoe-9.2.0-db-utils.zip` deliverable that can be manually applied by a Database Admin.
The provided DDLs allow creating from scratch the Database for the different {BAMOE} subsystems (see xref:deploying-process-services.adoc[The components of a process service]) for both PostgreSQL and Microsoft SQL Server
or migrating an existing database from a previous {PRODUCT_SHORT} version to the {PRODUCT_SHORT} version.

The provided scripts can be executed in any order, and the required tables will be created in the database that will be used by the {BAMOE} process service.
