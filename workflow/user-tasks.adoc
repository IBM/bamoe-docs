...
|*Example* 
a|`+curl -H "Content-Type: application/json" -H "Accept: application/json"
-X POST
...
|*Example* a|`+curl -H "Content-Type: application/json" -H "Accept: application/json"
-X POST
'http://localhost:8080/usertasks/instance/++{++taskId}/comments?user=++{++user}&group=++{++groups}'
-d '++{++comment: String}'+`
|===
