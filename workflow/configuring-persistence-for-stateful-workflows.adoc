...
When running multiple instances of a process service on multiple nodes in the compact architecture, you must assign a unique Job Service Management ID to each instance. This is to avoid multiple instances attempting to assume leadership over the Job Service and connection to the shared database and the failure of the service. 
...
[NOTE]
====
 We do not document tools and best practices for testing a deployment configuration or recovery in a multi-node setup. However we recommend thorough testing of scenarios such as pod failure, load balancing, and database replication prior to deploying in a multiple pod configuration.  
====