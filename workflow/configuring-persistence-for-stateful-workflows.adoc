...
[#multi-threading]
== Multi-threaded access to a business service using workflow functionality
In an environment with multi-threaded access to a business service using workflow functionality, the workflow engine needs to ensure proper state of the runtime data. If an entity, such as a process instance, is accessed concurrently by multiple threads, this can lead to race conditions. 
{PRODUCT_SHORT} supports proper handling of concurrent access of the same database entity, such as a process instance, through optimistic locking.
...
The recommendation is to enable optimistic locking through the following property in `application.properties`:
[source,properties]
----
