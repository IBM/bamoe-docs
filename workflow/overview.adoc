...
stateful process:: is a process that is able to persist its instance state into a database or other type of storage. It may include user tasks, timers, be long-running, have data tracking and auditing.


stateless process or straight-through process (STP):: is a process whose instance state cannot be stored in database. A stateless process, is executed in one transaction, does not have data persistence, timers or human tasks. Stateless processes are a subset of stateful process.
...
Human task:: is a subsystem that allows human tasks to be executed. It provides a lifecycle internally. This should not be confused with a user task node in the workflow. The user task node uses this subsystem to implement user tasks.
