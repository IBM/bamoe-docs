...
The Compact Architecture is the reference architecture for _Business Services_ containing stateful Workflows. In this architecture certain services (subsystems) can be colocated directly in your _Business Service_, simplifying configuration and minimizing communication between components, which ensures that _Business Services_ are stable and robust.
