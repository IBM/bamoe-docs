...
* The default User Task lifecycle (Kogito), see xref:#kogito-task-lifecycle[The Kogito User Task Lifecycle]
* The Human Task lifecycle (WS-Human Task Standard), see xref:#human-task-lifecycle[The Human Task Lifecycle]
 
You can override the default lifecycle using the following property to select the Human Task Lifecycle implementation if required:
...
If you make changes in the configuration of your _Business Service_ with workflow(s) to use WS-Human Task lifecycle, all active User Tasks will continue to complete their lifecycle with the default Kogito User Task lifecycle. Only user tasks created with the updated _Business Service_ would start using WS-Human task lifecycle.  
