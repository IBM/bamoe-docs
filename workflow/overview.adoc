= Overview of stateful Workflow architecture
...
The following table details the different components in Compact Architecture, indicating whether they are mandatory or not.
...
The User Task subsystem is in charge of the execution the User Tasks initiated by the workflow engine, allowing the user
drive its tasks through the different phases of their User Task Lifecycle until they reach a `Completion` state.

The User Task Subsystem provides the following features:

* Runs as a collocated service in the process engine with no extra configuration in the.
* Generic set of REST Apis to interact with it, allowing users transitioning tasks between the different lifecycle phases,
...
The User Task Subsystem defines a Default Lifecycle that allows user transition the task pass different phases that will change
the task state until it reaches a `Completion` state that will allow the associated process instance continuing its execution.
...
With the task *Reserved*, the owner will be able to complete the task (`Complete` phase) that will finally move te task in to a *Completed* that will
...
finalize the task in a *Obsolete* state denoting that the task hasn't been executed.
