...
+
This tool is useful to experiment and create your own queries to run against your application. You can use it in the development phase by setting `quarkus.kogito.data-index.graphql.ui.always-include=true`, and opening the GraphQL UI via `+http://localhost:8080/q/graphql-ui+`
+
...
[source,json]
----
mutation ProcessInstanceAbort($id : String){
...
[source,json]
----
mutation ProcessInstanceRetry($id : String){
  ProcessInstanceRetry(id : $id)
}
----
...
[source,json]
----
mutation ProcessInstanceSkip($id : String){
  ProcessInstanceSkip(id : $id)
}
----
...
[source,json]
----
mutation ProcessInstanceVariableUpdate($id : String, $variables : String){
  ProcessInstanceUpdateVariables(id : $id, variables : $variables)
}
----

=== Node Instance Trigger 

The `NodeInstanceTrigger` mutation enables you to trigger a node in a process instance. A node is a unit of work in a process definition, such as a task, action, or decision point. Triggering a node involves moving the process instance to the next state or node, effectively advancing the process. 
...
[source,json]
----
mutation NodeInstanceTrigger($id : String, $nodeID : String){
  NodeInstanceTrigger(id : $id, nodeID : $nodeID)
}
----
...
The following shows an example `NodeInstanceRetrigger` mutation which retriggers a node instance using the `id`, which is similar to `nodeInstanceId` related to a process instance. The `id` of the process instance is passed as a parameter. 

[source,json]
----
mutation NodeInstanceRetrigger($id : String, $nodeInstanceID : String){
  NodeInstanceTrigger(id : $id, nodeInstanceID : $nodeID)
}
----
...
[source,json]
----
mutation NodeInstanceCancel($id : String, $nodeInstanceID : String){
  NodeInstanceCancel(id : $id, nodeInstanceID : $nodeID)
}
----
...
You can this mutation to reschedule a specific job by providing its `id` and also by specifying the parameters `expirationTime`, `repeatInterval` & repeatInterval as the `data` argument.

[source,json]
----
mutation JobReschedule($id : String, $data : String) {
  JobReschedule(id : $id, data : $data)
}
----