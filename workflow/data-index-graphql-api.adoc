...
In GraphQL, filters are often handled through arguments passed to queries, and the conditions like logical operators (AND, OR, NOT) , between, contains, conatainsAll, containsAny,equalTo, greaterThan, greaterThanEqual, lessThan, lessThanEqual, isNull can be used to handle the complex query conditions.
...
In GraphQL, sorting is often implemented by using arguments in the query. The specific sorting mechanism and the way it's used depend on how the GraphQL schema is designed, but many GraphQL APIs allow sorting by providing arguments that specify the field(s) to sort by and the sort order (ascending or descending).To sort the value  based on a particular field in either ascending or descending order, you can use the ORDER BY clause in your Graphql query.

Can select a particular field and mention whether to sort the value in ascending (ASC) or descending (DESC) order of the parameter.
