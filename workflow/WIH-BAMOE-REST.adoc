...
Enhanced REST Service Call Task WorkItemHandler enables secure, authenticated REST API calls from BPMN workflows. The library provides a solution for integrating external REST services into your workflows with enhanced security and configuration capabilities.

You do not need to add the library to your project, because it is already included in all BAMOE workflow Accelerators. For more information about the Accelerators, see xref:..//tools/applying-accelerators.adoc[Using Accelerators]. The library is automatically configured and available for use in your workflows by xref:https://github.com/apache/incubator-kie-kogito-runtimes/tree/main/addons/common/rest[`org.kie.kogito:kogito-rest-workitem`] dependency included in BAMOE BOM. You can access the handler through BPMN Editor task palette without adding any additional dependencies.

To deploy the REST Service Call Task WorkItemHandler, you can use standard BAMOE deployment process or you can use no handler-specific deployment steps.
...
This section provides step-by-step guidance how to effectively use the REST Service Call Task and configure the REST Service Call Task WorkItemHandler in your BAMOE projects.
...
+
The dependency is automatically included in BAMOE BOM and all workflow accelerators. No manual dependency addition is required. For more information, see xref:https://github.com/apache/incubator-kie-kogito-runtimes/tree/main/addons/common/rest[`org.kie.kogito:kogito-rest-workitem`].
