...
+
NOTE: The endpoint is derived from Process Name, and each endpoint path must be unique. In order to enable multiple endpoints for different process versions, you must use a unique process name for each version. Having different version numbers is not sufficient to avoid potential issues. 
