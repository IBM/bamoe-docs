= Executing a stateful process service
include::../partials/attributes.adoc[]

This section describes the execution of a stateful process service and the difference in the deployment architecture to simplify event management and ensure stable and robust stateful applications. The standard Event-Driven {PRODUCT} setup is better adapted to Serverless execution.
...
Collocates Data Audit Subsystem within the {PRODUCT_SHORT} Runtime, and listens for the events fired by the Runtime, persisting it into Data Index.
...
Maven dependencies you will need to set to access the Job Service add-on:
