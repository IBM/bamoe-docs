...
Compact Architecture consists of applications built using a new set of {PRODUCT} add-ons, that collocate add-on services (Jobs, Data audit, Data index) in the runtime.
...
When deploying process service, you will need to configure the database so that the process runtime can persist data such as User Tasks, Audit logs, etc. To do that, {PRODUCT_SHORT} comes with a Flyway-based mechanism that automatically creates the database schema during startup. This is convenient but it is often the case that you will need more control over when and how scripts are run in production database instances.

A set of DDL (Data Definition Language) scripts is included in the delivery (`bamoe-9.2.0-db-utils.zip`) to enable a Database administrator to manually execute them as required on a production database, in order to create the database to be used by the persistence layer of the following subsystems:
...
The script creates from scratch, all the tables required by the above features to work. 
...
|definitions 
|process definitions that has been deployed
