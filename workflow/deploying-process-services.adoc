= Deploying a stateful workflow service
...
{PRODUCT_SHORT} process service and all the {PRODUCT_SHORT} subsystems (Data-Index, Data Audit, Jobs Service, and User Tasks) use JDBC to persist
...
==== {PRODUCT_SHORT} Microsoft SQL Server Mappings
...
The provided DDLs allow creating from scratch the Database for the different {PRODUCT_SHORT} subsystems (see xref:deploying-process-services.adoc[The components of a process service])
...
|`kogito.transactionEnabled` |It globally sets the transactions enabled for the entire application |Boolean |true

|`kogito.processes.transactionEnabled` |It sets the transactions enabled for the general Process API layer |Boolean |true

|`kogito.usertasks.transactionEnabled`|It sets the Transactions enabled for the new User Tasks API layer |Boolean |true
|===
