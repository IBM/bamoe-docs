...
As the Quarkus add-on is vendor independent you will need to add the `io.quarkus:quarkus-messaging-kafka` dependency to be able to use Kafka as the message broker. For more information, refer to the tool's external documentation.
...
The Spring Boot add-on is Kafka-specific therefore you will only need to add the `org.kie:kie-addons-springboot-messaging` dependency to enable messaging. For more information, refer to the tool's external documentation.
...
In Spring Boot, you configure process event capabilities for your _Business Services_ by adding the `org.kie:kie-addons-springboot-events-process-kafka` dependency. For more information, refer to the tool's external documentation.
