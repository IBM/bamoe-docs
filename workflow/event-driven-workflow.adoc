...
In the following cases you will want to use the the Correlation feature:

* The message sender does not know or need to know the specific Process Instance ID
* The matching key is a business identifier already present in the message payload and not an internal engine ID
* You want loose coupling where routing logic is in the BPMN Process Definition and not in the message producer. In this situation the message producer sends domain events and the engine decides where they land.

Correlation uses three BPMN components to route messages to the correct Process Instance at runtime:
...
. *Correlation Subscription* - Defines the expected value for matching with a running Process Instance
+
...
When you have multiple Process Instances waiting for the same intermediate catch event you can use the `kogitoprocrefid` field to route incoming messages to a specific waiting Process Instance. The value must be the Process Instance ID of the process that is currently waiting. The sender must have obtained ID previously, for example, from a start-event response, REST API call or external storage. Without `kogitoprocrefid`, the message will be delivered to any waiting Process Instance resulting in unintended consequences.
...
. Handle exceptions gracefully, always wrapping external calls in try-catch blocks
+
NOTE: Event listeners execute within the same transaction as the task or process operation. If your listener throws an uncaught exception, it will roll back the entire transaction — including the user task completion or process state change.
...
For complete working examples demonstrating `EventPublisher` implementations, see the `process-event-listeners-quarkus` and `process-event-listeners-springboot` examples in the {PRODUCT_SHORT} examples repository xref:../getting-started/examples.adoc[Getting Started Example _Business Service_ projects].
