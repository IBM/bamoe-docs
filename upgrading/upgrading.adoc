...
== Step 3a: Creating a {PRODUCT_SHORT} version {VERSION} _Business Service_ project from scratch
...
[#step-3b]
== Step 3b: Upgrade {PRODUCT_SHORT} versions {VERSION_90X} or {VERSION_91X} _Business Service_ projects

Upgrading a {PRODUCT_SHORT} version {VERSION_90x} or {VERSION_91x} _Business Service_ project to version {VERSION} is straightforward.
...
[start=2]
. Upgrade the BAMOE BOM to in the project POM file to {BOM_VERSION_920}:
...
== Migrating stateful Workflows from {PRODUCT_SHORT} version {VERSION_911} technology preview
