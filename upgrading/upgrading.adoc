...
You can upgrade your existing business automation applications and platforms to the most recent version of {PRODUCT_LONG}. The supported upgrade procxersion, you must follow the steps to upgrade to each intermediate version, as documented. Depending on the complexity of your installation, you might want IBM(R) Services to participate in your project. If so, contact your IBM service representative.
To know about upgrading to version 9.0, see [Link to 90x upgrading topic]

== Prerequisites

The following tools are required to upgrade to {PRODUCT_SHORT} version {VERSION_90X}:
