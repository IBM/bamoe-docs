...
- https://marketplace.visualstudio.com/items?itemName=IBM.bamoe-developer-tools[{DEV_TOOLS}] latest version of VS Code IDE extensions
...
To set this option, go to Visual Studio Code, click Extensions and then select the {DEV_TOOLS}. You will see a checkbox 'Auto Update' on the page. Click it to set. This will take care of your updates automatically. To know more about {DEV_TOOLS}, see xref:../tools/developer-tools-for-vscode.adoc[Developer Tools for VS Code].
