...
* {PRODUCT_SHORT} project(s)

* {PRODUCT_SHORT} production environment installation with Management and Task console, and upgraded versions of your {PRODUCT_SHORT} projects.

If you are using the {VERSION_90X} technology preview for stateful Workflows, you will find instructions on what to do to migrate your stateful workflow projects in xref:#migrate-tech-preview[this section].

//Remember to change the technology preview link after creating the topic.
...
In case you have installed {MVN_REPO} in a Maven Repository Manager such as Artifactory or Nexus, follow the steps in xref:../installation/configure-maven.adoc[Installing → {MVN_REPO}] to install the new {MVN_REPO} version {VERSION_91X}.

=== Upgrade {CANVAS}

Before you update {CANVAS}, save your existing projects to the browser.
...
== Step 3: Upgrade your {PRODUCT} project(s)

{PRODUCT} project(s) can be upgraded in two ways, by:

* Updating an existing BAMOE version {VERSION_90X} project with its artifacts by changing the Maven dependencies. 

* Creating a new project from scratch and moving your existing assets to it.

The following tools are required to upgrade your {PRODUCT_SHORT} projects to version {VERSION_91X}:
...
[NOTE]
====
After completing the upgrade of your {PRODUCT_SHORT} project, you might want to consider upgrading your business Rules to use new DRL features, such as `rule units` and `OOPATH syntax`.
...
== Step 3a: Upgrade {PRODUCT_SHORT} project(s) on version {VERSION_90X}

Once your development environment installation is upgraded, it is straightforward to upgrade your project(s) on {PRODUCT_SHORT} {VERSION_90x} to {PRODUCT_SHORT} version {VERSION_91x}.
...
[WARNING]
====
If you come across some difficulties when trying to upgrade your existing {PRODUCT_SHORT} projects, you are recommended to create a new project following xref:#step-3b[Step 3b: Upgrade {PRODUCT_SHORT} projects on versions {VERSION_90X}].
====
...
[start=2]
. Upgrade the BAMOE BOM in the project POM file to {BOM_VERSION_911}:
+
...
== Step 3b: Creating a {PRODUCT_SHORT} project on version {VERSION_91x} from scratch

If you have experienced some difficulties when trying to upgrade your project you can create a new {PRODUCT_SHORT} project from scratch and import your existing business automation assets (Workflows, Decisions, Rules, and Decision Tables) to the new project.

For more information, see xref:../getting-started/project-setup.adoc[Getting started → Initial Business Service project setup and walkthrough] to create a new project on {PRODUCT_SHORT} version {VERSION_91x}.
...
== Step 4: Verify the upgrade of your {PRODUCT_SHORT} project
...
----
+
Now you can start using your project on {PRODUCT_SHORT} version {VERSION_91X}.

== Step 5: Upgrade your {PRODUCT_SHORT} production environment installation

After upgrading your source code (Steps 3a or 3b), you need to upgrade {MANAGEMENT_CONSOLE} before deploying a new version of the upgraded {PRODUCT_SHORT} projects.

It is recommended that you use the https://docs.openshift.com/container-platform/4.16/applications/deployments/route-based-deployment-strategies.html#deployments-blue-green_route-based-deployment-strategies[blue-green deployment strategy] for upgrading {MANAGEMENT_CONSOLE} so that you still have a reliable way of managing live projects that might not have been upgraded to {PRODUCT_SHORT} version {VERSION_91X} yet. Once all your projects are running, you can safely remove your previous {MANAGEMENT_CONSOLE} installation.
...
Once you have a new installation for {MANAGEMENT_CONSOLE}, you can follow the standard guidance on how to deploy a new version of your projects (see xref:../runtime-services-modeling/services-build.adoc[Building Business Services executables] and xref:../runtime-services-modeling/services-deployment.adoc[Deploying Business Services]).
...
== Step 3b: Upgrading a version {VERSION_90x} project
