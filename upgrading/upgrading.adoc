= Upgrading from BAMOE version 9.1.x
...
| IBM BAMOE Developer Tools for VS Code to create, edit and visualize business processes (BPMN), decisions (DMN), test scenarios (SceSim), and generate web forms used during workflow execution.
...
| This package contains the 'docker-compose' file to run Business Automation Manager Open Edition {VERSION} Canvas and all related services.
...
The {VS_CODE_BUNDLE} will automatically update as long as you have set this option in VS Code.

To set this option, go to Visual Studio Code, click Extensions and then select the {VS_CODE_BUNDLE}. You will see a checkbox 'Auto Update' on the page. Click it to set. This will take care of your updates automatically. To know more about {VS_CODE_BUNDLE}, see xref:../tools/developer-tools-for-vscode.adoc[Developer Tools for VS Code].
...
You need to update your business service and management console. 
...
To update your management console installation, see xref:../workflow/consoles.adoc[consoles].
...
The Returned JSON object contains more detailed data. That means, you need to adapt how consuming the entrypoint data according to the returned data.

Therefore, in most of the case, migrating to the new API requires:

Here, you can find a description on how migrating to the new API based
...
The JSON returned by the User Task API to represents a Task is significantly different from the JSON returned by the Process API.

Here, an example of the two returned objects for both endpoints. The new User Tasks API returns:
...
Task Information provided by the New User Tasks API

While the Current Process API returns: 
...
Task Information provided by the Current Process API

Those differences consist in:

As underlined previously, User Tasks IDs are not the same.

The assigned ID in the for the Task in the new User Tasks API
(e447c247-abc2-4ed4-a7ab-ef4ee2fe4a71) is scoped inside the User Tasks
subsystem. That means that id can be used in that subsystem only, and it’s the ID to use as taskID parameter in all the
entrypoints defined in the new User Tasks API. The rest of the engine is
out-of-scope, so using that ID in the current Process API entrypoint
will lead to an error.

* *The externalReferenceId element represents the Work Item ID, that is
how the Process API identifies this User Task at global level. That is,
in fact, the returned ID for the User Task using the current Process
API. *
* *Several new data related to the assigned permissions of the Tasks
have been added, like the actual owner, potential users and groups and
excluded users*
* *The parameters object present in the old API is now renamed to input.
This represents a Map of String, Objects entries that the user can set
as the User Task input. It’s possible to modify this list adding more
entries with a specific entrypoint*
* *The parameters object present in the old API is now renamed to
output. This represents a Map of String, Objects entries that the user
can set as the User Task output. It’s possible to modify this list
adding more entries with a specific entrypoint*
* *The new API Task object now contains a Metadata object. This contains
relevant properties that describes the BPMN asset *

*Having a dedicated API for User Tasks, it is now possible to directly
put new Input and Output data for a given User Task, using the following
entrypoints: (with the previous Process API that was not possible).*
...
|*Process API* |Not possibile to modify inputs in the previous API
version.
...
|*Process API* |It was not possibile to pass Output data to a User Task
when sending a termination (eg. completion) phase update call.
...
*Now, some examples on how to manage Comments and Attachments*
...
Attachments. Please refer to the previous paragraph for the precise
endpoint definitions.*

*As the last use case, here is shown how to apply a transition to a
given User Task.* +
*The transition scope is to advance a User Task status, moving it from
one status to another (transitionID), which can be ("activate", "claim",
"release", "complete", "skip", or “fail”). The following example shows
...
*As you may notice, with Process API there are two ways to change the
...
“complete”. It’s possible to pass output values in the same call (E.g.,
“approved” = true)*

*The new User Task API requires that the transitionID is defined in the
JSON payload, as in the example below. It’s not possible anymore to pass
output values like in Process API, so if you need to put some output
data, you need to call the Add Output entypoint described some examples
below.*
