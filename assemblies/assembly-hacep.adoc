...
////
You cannot use a standard deployment of {PRODUCT} on {OPENSHIFT}, as described in {URL_DEPLOYING_ON_OPENSHIFT}/assembly-openshift-operator.html[_{DEPLOYING_OPENSHIFT_OPERATOR}_], to implement high available event-driven decisioning, because the standard deployment supports only stateless processing. You must therefore create a custom implementation using the provided reference implementation.
////
