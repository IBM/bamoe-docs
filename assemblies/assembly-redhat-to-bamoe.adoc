...
This section provides guidance on upgrading from {RH_PAM} or {RH_DM} to {PRODUCT} {VERSION_80}. It is recommended to complete this upgrade path before moving to the next generation of the product in {PRODUCT_SHORT} 9.4.x.

If you have designed your jBPM application using KIE Server with Spring Boot and want to move from the latest community version to the enterprise-supported {PRODUCT_SHORT} 8.0.x, see https://community.ibm.com/community/user/automation/blogs/tim-wuthenow/2023/06/16/community-to-enterprise-bamoe[Moving from Community to Enterprise]. Both projects use Maven-based architectures, which simplifies the migration process.

If your current application is based on {RH_BPMS}, you must first upgrade to {RH_PAM}/{RH_DM} 7.x before moving to {PRODUCT}. For more information, See the official https://access.redhat.com/documentation/en-us/red_hat_process_automation_manager/7.0/html/migrating_from_red_hat_jboss_bpm_suite_6.4_to_red_hat_process_automation_manager_7.0/index[Red Hat Documentation].

If you are currently on RHPAM/RHDM 7.13.5, you can upgrade directly to any {PRODUCT_SHORT} 8.0.x release. For older versions of RHPAM/RHDM (prior to 7.13.5), you must first upgrade to RHPAM/RHDM 7.13.5 and then proceed to any {PRODUCT_SHORT} 8.0.x release.
...
Download the following files _(replace with the most current version)_ based on your requirement:

* **bamoe-{version}-add-ons.zip**: Contains additional product components needed in specific situations, including:
...
//Your project data and settings in Business Central (stored in the `.niogit` directory) can be migrated using the migration tooling (see xref:#niogit_directory[Using migration tool to prepare Business Central `.niogit` directory for upgrade]). 

//To install the new versions of decision engine, Business Central, KIE Server, and the headless controller in a duplicate environment, follow the instructions in xref:../installing-and-configuring.html[Installing and configuring].

//=== Prerequisites

//Shut down any Business Central and Kie Server instances and back up your production environment if you are updating it directly.
...
//. In your command terminal, navigate to the temporary directory where you extracted the `bamoe-$VERSION-update.zip` file and run the _apply-updates_ script in the following format (Ensure  Decision/Business Central and KIE Server instances are not running):
...
//| Archive | Description
//
//| bamoe-business-central-eap7-deployable   
//| Updates Business Central (`business-central.war`)
//
//| bamoe-kie-server-ee8
...
//+
//
//. After the update tool completes, return to the {PRODUCT_DOWNLOAD_LINK}[IBM Support] page where you have downloaded the update tool and install any other updates or new release artifacts that are relevant to your {PRODUCT} distribution.
//
//+
//For files that already exist in your {PRODUCT} distribution, such as .`jar` files for the decision engine or other add-ons, replace the existing version of the file with the new version from the {PRODUCT_DOWNLOAD_LINK}[IBM Support] page.
//
...
* The `bamoe-version-add-ons.zip` must be downloaded which contains the migration tool.
...
. In your command terminal, copy the `.niogit` directory from your current Business Central distribution for Red Hat Process Automation Manager 7.13.5 to the bin folder of the new {PRODUCT_SHORT} 8.0.x installation directory:
+
[source,shell]
----
