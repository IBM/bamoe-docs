...
+
.  In the {PRODUCT_SHORT} 8.0.x bin directory, delete the `.index` directory. You must delete this folder because {PRODUCT_SHORT} 8.0.x comes with certain updates that are not compatible with Red Hat Process Automation Manager 7.4. The bin directory will be re-indexed after the migration when you start {PRODUCT_SHORT} 8.0.x. 
