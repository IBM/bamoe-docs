...
This section provides guidance on upgrading from {RH_PAM} or {RH_DM} to {PRODUCT} {VERSION_80}. It is recommended to complete this upgrade path before moving to the next generation of the product in BAMOE version 9.1.x.
...
If your current application is based on {RH_BPMS}, you must first upgrade to {RH_PAM} or {RH_DM} version 7.x before moving to {PRODUCT}. See the official https://access.redhat.com/documentation/en-us/red_hat_process_automation_manager/7.0/html/migrating_from_red_hat_jboss_bpm_suite_6.4_to_red_hat_process_automation_manager_7.0/index[Red Hat Documentation] for upgrading from {RH_BPMS} to {RH_PAM} or {RH_DM} version 7.x.  

If you are currently on {RH_PAM} or {RH_DM} 7.13.5, you can upgrade directly to any {PRODUCT_SHORT} 8.0.x release. For older versions of {RH_PAM} or {RH_DM} (prior to 7.13.5), you must first upgrade to {RH_PAM} or {RH_DM} 7.13.5 and then proceed to any {PRODUCT_SHORT} 8.0.x release.

.Upgrading from Red Hat to {PRODUCT}
image::assembly/bamoe-upgrade-flowchart.png.png[]
