...
It is recommended that you follow this upgrade path before attempting to upgrade to the next generation of the product in {PRODUCT} version 9.1.x.
...
If your current application is based on the legacy product known as {RH_BPMS}, for an in-place upgrade you must first bring your installation up to the latest release of {RH_PAM} / {RH_DM} version 7 before performing any upgrade to {PRODUCT}.
...
After this upgrade, you are free to upgrade to any release of {PRODUCT_SHORT}.

The following tools are required to upgrade to {PRODUCT_SHORT} version {VERSION_80}:

- https://openjdk.org[JDK 11] is installed.
...
| bamoe-8.0-BC7   
| Updates Business Central (`bamoe-business-central-eap7-deployable.war`)

| bamoe-8.0-KS8
| Updates KIE Server (`bamoe-kie-server-ee8.war`)
...
[source,shell,subs="attributes+"]
----
$ ./apply-updates.sh ~EAP_HOME/standalone/deployments/bamoe-business-central-eap7-deployable.war bamoe-8.0-BC7
$ ./apply-updates.sh ~EAP_HOME/standalone/deployments/bamoe-kie-server-ee8.war bamoe-8.0-KS8
----
