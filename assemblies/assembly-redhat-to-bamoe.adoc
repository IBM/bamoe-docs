...
Your project data and settings in Business Central (stored in the `.niogit` directory) can be migrated using the migration tooling (see xref: #niogit_directory).To Install the new versions of Business Central, KIE Server, and the headless controller in a duplicate environment, follow the instructions in xref:../doc-content/enterprise-only/installation/installing-and-configuring.adoc[Installing and configuring].
