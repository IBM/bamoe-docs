...
Your project data and settings in Business Central (stored in the `.niogit` directory) can be migrated using the migration tooling (see xref: #niogit_directory). Install new versions of Business Central, KIE Server, and the headless controller in a duplicate environment following instructions in xref:installing-and-configuring.html[Installing and configuring].
Other {PRODUCT_SHORT} artifacts such as decision engine and standalone Decision/Business Central, are released as new artifacts with each minor release and you must reinstall them to apply the update. You can use the same automated update tool to apply both patch updates and minor release upgrades.
