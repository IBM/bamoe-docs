...
This section provides guidance on upgrading from {RH_PAM} or {RH_DM} to {PRODUCT} {VERSION_80}. It is recommended to complete this upgrade path before moving to the next generation of the product in {PRODUCT_SHORT} 9.3.x.
...
Your project data and settings in Business Central (stored in the `.niogit` directory) can be migrated using the migration tooling (see xref:#niogit-directory). To Install the new versions of Business Central, KIE Server, and the headless controller in a duplicate environment, follow the instructions in xref:../doc-content/enterprise-only/installation/installing-and-configuring.html[Installing and configuring].
...
1. xref:../doc-content/enterprise-only/installation/installing-and-configuring.html[Installing and configuring]
2. xref:doc-content/enterprise-only/installation/installing-and-configuring.html[Installing and configuring]
3. xref:installing-and-configuring.html[Installing and configuring]
4. xref:../../installation/installing-and-configuring.html[Installing and configuring]
5. xref:../../../installing-and-configuring.html[Installing and configuring]
5. xref:../installing-and-configuring.html[Installing and configuring]
...
* System configuration directory structure migration: Migrates the `system.git` repository structure used in Red Hat Process Automation Manager 7.13.5 to the new structure used in {PRODUCT_SHORT} 8.0.x. This migration option requires the project structure migration to be executed first.
...
* POMs migration: Updates `pom.xml` files with dependencies required for {PRODUCT_SHORT} 8.0.x. This migration option requires the project structure migration, system configuration directory structure migration, and forms migration to be executed first.
...
+
NOTE: If you prefer to run one migration option at a time, select and run the first individual migration option. After the tool runs, re-run the Business Central migration tool and select the next individual migration option in the sequence. 
+
