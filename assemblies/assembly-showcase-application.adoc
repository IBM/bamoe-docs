...
* {PRODUCT} is installed on {EAP} and configured with {KIE_SERVER}. For more information see xref:assembly-install-on-eap.html[_{INSTALLING_ON_EAP}_].
* `KieLoginModule` is configured in `standalone-full.xml`. This is required to connect to {KIE_SERVER}. For more information about configuring {KIE_SERVER}, see xref:assembly-planning.html[_{PLANNING_INSTALL}_].
* {PRODUCT} is running and you can log in to {CENTRAL} with a user that has both `kie-server` and `user` roles. For more information about roles, see xref:assembly-planning.html[_{PLANNING_INSTALL}_].
