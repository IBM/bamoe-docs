...
* {PRODUCT} is running and you can log in to {CENTRAL} with the `process-admin` role. For more information, see xref:assembly-planning.html[_{PLANNING_INSTALL}_].
...
include::{enterprise-dir}/admin-and-config/postgresql-largeobject-cleanup-proc.adoc[leveloffset=+2]
