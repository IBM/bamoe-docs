...
{PRODUCT} provides runtime support for DMN 1.1, 1.2, 1.3, and 1.4 models at conformance level 3, and design support for DMN 1.2 models at conformance level 3. You can design your DMN models directly in {CENTRAL} or with the {PRODUCT} DMN modeler in {VS_CODE}, or import existing DMN models into your {PRODUCT} projects for deployment and execution. Any DMN 1.1 and 1.3 models (do not contain DMN 1.3 features) that you import into {CENTRAL}, open in the DMN designer, and save are converted to DMN 1.2 models.
...
For a step-by-step tutorial with an example DMN decision service, see xref:assembly-getting-started-decision-services.html[_{GETTING_STARTED_DECISION_SERVICE}_].
