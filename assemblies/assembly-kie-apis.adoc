...
[NOTE]
====
The endpoints and APIs, in particular the Process Admin Service Management API, are intended to be used only through the REST endpoints or the Java Client API. The invocation of these endpoints can alter the state of a live process instance and should be used with caution. Calling these methods from within a process definition itself can cause unforeseen issues and is therefore not recommended.
====


.Prerequisites
* {PRODUCT} is installed and running. For installation and startup options, see xref:assembly-planning.html[_{PLANNING_INSTALL}_].
...
+
Although these user roles are not all required for every KIE API, consider acquiring all of them to ensure that you can access any KIE API without disruption. For more information about user roles, see xref:assembly-planning.html[_{PLANNING_INSTALL}_].
...
* xref:assembly-managing-and-monitoring-execution-server.html[_{MANAGING_KIE_SERVER}_]
* xref:assembly-packaging-deploying.html[_{PACKAGING_DEPLOYING_PROJECT}_]
