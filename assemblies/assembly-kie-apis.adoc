...
.Prerequisites
* {PRODUCT} is installed and running. For installation and startup options, see {URL_INSTALLING_AND_CONFIGURING}/assembly-planning.html[_{PLANNING_INSTALL}_].
...
+
Although these user roles are not all required for every KIE API, consider acquiring all of them to ensure that you can access any KIE API without disruption. For more information about user roles, see {URL_INSTALLING_AND_CONFIGURING}/assembly-planning.html[_{PLANNING_INSTALL}_].
...
* {URL_MANAGING_SETTINGS}/assembly-managing-and-monitoring-execution-server.html[_{MANAGING_KIE_SERVER}_]
