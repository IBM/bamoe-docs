...
* {PRODUCT} is installed and configured with {KIE_SERVER}. For more information see xref:assembly-install-on-eap.html[_{INSTALLING_ON_EAP}_].
//xreflink
* {PRODUCT} is running and you can log in to {CENTRAL} with the `developer` role. For more information, see xref:assembly-planning.html[_{PLANNING_INSTALL}_].
