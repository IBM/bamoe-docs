...
It is also very useful for quickly evaluating a set of inputs.
...
Let us create a new Decision from one of the Samples provided by {CANVAS}.
On the Home screen, select “Try sample” on the Decision card.
When you click the “Run” button, you will see the screen below.
Let us see what those elements are.
...
Let us add a new input row, switch to Table mode, and look at the DMN Runner elements.
...
If you are familiar with the DMN Boxed Expression Editor, you will be very comfortable navigating through the DMN Runner Table, as they share the same Table component.
...
* Recursive Struct Data Types are not yet supported.
  You will see a placeholder component where the inputs should be if you have one of those in your Decision.
