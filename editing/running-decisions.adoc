...
The evaluation highlights feature helps to visualize used evaluation branches of Decision Tables and Boxed Conditional Expressions.


=== Evaluation highlights - enable or disable
The user have two possibilities to enable or disable the evaluation highlights feature.
Fisrt option is the overlays panel.
Toggle the enable evaluation highlights button to enable or disable the feature.
Once the feature is enabled, there will be indicator in the top right corner.

.Evaluation highlights enabled
image::tools/runningDecisions/evaluation-highlights-enabled-indicator-enabled.png[]

.Evaluation highlights disabled
image::tools/runningDecisions/evaluation-highlights-enabled-indicator-disabled.png[]


=== Evaluation highlights - nodes
Once the feature is enabled, all decision nodes will have an evalaution result badge in its top left corner.
Such a badge has an icon representing the evaluation result of the node.
...
All needed inputs were provided and no errorw was thrown during the evaluation.

.Failure
image::tools/runningDecisions/evaluation-highlights-node-failure.png[]
All needed inputs were provided however an error was thrown during the evaluation.

.Skipped
image::tools/runningDecisions/evaluation-highlights-node-skipped.png[]
Not all needed inputs were provided.

=== Evaluation highlights - expressions
For decision nodes that are defined either as decision table or as an conditional expression, the feature will show to the user row evaluation hit count.

For the DMN Runner form mode, the evaluation hits count will be mostly zero or one.
Using the DMN Loan Application sample and these inputs:
...
.Loan Pre-Qualification evaluation hits count - form mode
image::tools/runningDecisions/evaluation-highlights-loan-pre-qualification-form-mode.png[]

For the DMN Runner table mode, the evaluation hits count may be greater than one.
Using the DMN Loan Application sample and these inputs in DMN Runner table mode will produce this:
...
.Loan Pre-Qualification evaluation hits count - table mode
...
There is source code available for the https://raw.githubusercontent.com/bamoe/bamoe-docs-source-code-examples/{VERSION_X}/dmn/payroll.dmn[payroll.dmn] example model.
