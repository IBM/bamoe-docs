...
For more information about DMN, see the Object Management Group (OMG) https://www.omg.org/spec/DMN[Decision Model and Notation (DMN) specification].
...
For more information about the background and applications of DMN, see the OMG https://www.omg.org/spec/DMN[Decision Model and Notation (DMN) specification].
...
All Friendly Enough Expression Language (FEEL) expressions that you use in your boxed expressions must conform to the FEEL syntax requirements in the OMG https://www.omg.org/spec/DMN[Decision Model and Notation (DMN) specification].
...
There is available source code of the {BAMOE_DOCS_EXAMPLES_DOCS_RAW_LINK}/dmn/flight-has-capacity.dmn[Filter expression of rebooked flights using item keyword] example model.
...
[role="_abstract"]
By default, the _Business Services_ use the `kogito.decisions.validation=ENABLED` configuration to perform pre-compilation validation of DMN model assets and to perform DMN Decision Table static analysis.
...
Any DMN validation error causes the _Business Services_ code-gen build step to fail and errors are reported accordingly, as shown in the following example:
...
NOTE: If you enter an unrecognized `kogito.decisions.validation` configuration flag, all pre-compilation validation features are disabled and the _Business Services_ code-gen emits related log messages.
...
== Creating and editing DMN models in the {PRODUCT} DMN Editor

[role="_abstract"]
You can use the {DEV_TOOLS_VSCODE_MARKETPLACE_LINK}[*{DEV_TOOLS}*] to design DMN decision requirements diagrams (DRDs) and define decision logic for a complete and functional DMN decision model.

.Procedure
. In your VS Code IDE, create or import a DMN file in the relevant folder of your _Business Service_ project, typically in `src/main/resources`.
. Open the new or imported DMN file to view the decision requirements diagram (DRD) in the {PRODUCT} DMN Editor.
+
--
...
--
. In the DMN Editor diagram, double-click the new DRD node to enter an informative node name.
...
. For the selected DRD node, use the available connection options to create and connect to the next node in the DRD, or click and drag a new node onto the DRD from the left toolbar.
+
--
...
=== Defining DMN decision logic in boxed expressions in the {PRODUCT} DMN Editor

[role="_abstract"]
Boxed expressions in DMN are tables that you use to define the underlying logic of decision nodes and business knowledge models in a decision requirements diagram (DRD). Some boxed expressions can contain other boxed expressions, but the top-level boxed expression corresponds to the decision logic of a single DRD artifact. While DRDs represent the flow of a DMN decision model, boxed expressions define the actual decision logic of individual nodes. DRDs and boxed expressions together form a complete and functional DMN decision model.

You can use the {PRODUCT} DMN Editor in VS Code to define decision logic for your DRD components using built-in boxed expressions.

.Procedure
. In your VS Code IDE, open the DMN file to view the decision requirements diagram (DRD) in the {PRODUCT} DMN Editor.
+
--
include::../partials/vs-code-bundle-precondition.adoc[]
--
. In the DMN Editor, select a decision node or business knowledge model node that you want to define and click the *Edit* icon to open the DMN boxed expression editor:
+
--
.Opening a new decision node boxed expression
image::dmn-decision-edit.png[]
...
For information about constraint types and syntax requirements for the specified data type, see the https://www.omg.org/spec/DMN[Decision Model and Notation (DMN) specification].
--
...
include::../partials/vs-code-bundle-precondition.adoc[]
--
. In the DMN Editor diagram, select a decision node or business knowledge model for which you want to define the data types and click the *Edit* icon to open the DMN Boxed Expression Editor.
...
You can also set and manage custom data types for a specified decision node or business knowledge model node by selecting the *Properties* icon in the upper-right corner of the DMN Editor:
...
include::../partials/vs-code-bundle-precondition.adoc[]
--
. In the DMN Editor, click the *Included Models* tab.
...
--
. In the *Editor* tab of the DMN Editor, click and drag the included DRD components onto the diagram to begin implementing them in your DRD:
+
--
...
To edit the included model name or to remove the included model from the DMN file, use the *Included Models* tab in the DMN Editor.
...
==== Including PMML models within a DMN file in the {PRODUCT} VS Code DMN Editor

You cannot include PMML models from different directories within your _Business Service_ project or from other _Business Service_ projects.

[NOTE]
====
Please notice {PRODUCT} doesn't contain a PMML modeler. You need to create the PMML model before it is included into DMN model using some other PMML modeler. For example the Apache KIE community https://marketplace.visualstudio.com/items?itemName=kie-group.pmml-vscode-extension[PMML Scorecard Modeler].
====
...
--
. In the *Editor* tab of the DMN Editor, select or create the decision node or business knowledge model node in which you want to invoke the PMML model and click the *Edit* icon to open the DMN boxed expression editor:
+
--
.Opening a new decision node boxed expression
image::dmn-decision-edit.png[]
...
=== Creating DMN models with multiple diagrams in {DEV_TOOLS}

[role="_abstract"]
For complex DMN models, you can use the {PRODUCT} DMN Editor in VS Code to design multiple DMN decision requirements diagrams (DRDs) that represent parts of the overall decision requirements graph (DRG) for the DMN decision model. In simple cases, you can use a single DRD to represent all of the overall DRG for the decision model, but in complex cases, a single DRD can become large and difficult to follow. Therefore, to better organize DMN decision models with many decision requirements, you can divide the model into smaller nested DRDs that constitute the larger central DRD representation of the overall DRG.

.Procedure
. In your VS Code IDE, create or import a DMN file in the relevant folder of your _Business Service_ project, typically in `src/main/resources`.

. Open the new or imported DMN file to view the DRD in the {PRODUCT} DMN Editor, and begin designing or modifying the DRD using the DMN nodes in the left toolbar.
+
--
include::../partials/vs-code-bundle-precondition.adoc[]
--
...
After you create a nested DRD within your DMN decision model, the new DRD opens in a separate DRD and the available DRD and components are listed in the *Decision Navigator* right menu. You can use the *Decision Navigator* menu to rename or remove a nested DRD.

.Rename new nested DRD in the Decision Navigator menu
image::dmn-drd-actions-rename.png[]
--
. In the separate diagram for the new nested DRD, design the flow and logic for all required components in this portion of the DMN model, as usual.
...
=== DMN model documentation in the {PRODUCT} DMN Editor

[role="_abstract"]
In the {PRODUCT} DMN Editor, you can use the *Documentation* tab to generate a report of your DMN model. The DMN model report contains all decision requirements diagrams (DRDs), data types, and boxed expressions in your DMN model. You can use this report to share your DMN model details or as part of your internal reporting workflow.

.Example DMN model report
image::dmn-documentation.png[]

[id="ref-dmn-designer-nav"]
=== {PRODUCT} DMN Editor navigation and properties

[role="_abstract"]
The {PRODUCT} DMN Editor provides the following additional features to help you navigate through the components and properties of decision requirements diagrams (DRDs).

DMN decision and diagram views::
In the upper-right corner of the DMN Editor, select the *Decision Navigator* view to navigate between the decision components, graphs, and boxed expressions of a selected DRD:
+
--
...
In the upper-right corner of the DMN Editor, select the *Preview* icon to view an elevated preview of the DRD:
...
In the upper-right corner of the DMN Editor, select the *Properties* icon to modify the identifying information, data types, and appearance of a selected DRD, DRD node, or boxed expression cell:
+
--
.DRD node properties
image::dmn-designer-properties.png[]

To view the properties of the entire DRD, click the DRD background instead of a specific node.
--

DRD search::
In the upper-right corner of the DMN Editor, use the search bar to search for text that appears in your DRD. The search feature is especially helpful in complex DRDs with many nodes:
+
.DRD search
