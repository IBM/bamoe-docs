...
* Using the terminal
* Using the *Run Java* on VS Code when you have the *Language Support for Java(TM) by Red Hat* extension installed


== Using the terminal

Open a terminal in the root directory of your _Business Service_ and execute the following command:
...
Check the results to ensure the scenarios pass. The test results are visible in your terminal after the completion of the command.
...
Open the `KogitoScenarioJunitActivator.java` file, then click the *Run Java* button. 

Test results appear in the native VS Code *Test Results* window.

image::developer-tools/scesim-java-test-runner-results.png[]

During the Maven test phase, all Test Scenarios defined in `.scesim` files are executed as a single unit test, and the results are saved in the `target/surefire-reports` folder. In Gradle, the test results are saved in the `build/test-results/test` folder.

All other regular unit tests and integration tests are also executed, and their results are stored in the same respective folders.
...
// [source]
// ----
// [INFO] --- maven-surefire-plugin:3.5.3:test (default-test) @ sample-kogito ---
// [INFO]
// [INFO] -------------------------------------------------------
// [INFO]  T E S T S
// [INFO] -------------------------------------------------------
...
// [INFO] Finished at: 2025-05-05T15:19:53-04:00
// [INFO] ------------------------------------------------------------------------
// ----
...
// [INFO] Finished at: 2025-05-05T15:26:10-04:00
// [INFO] ------------------------------------------------------------------------
// [ERROR] Failed to execute goal org.apache.maven.plugins:maven-surefire-plugin:3.5.3:test (default-test) on project sample-kogito: There are test failures.
// [ERROR]
