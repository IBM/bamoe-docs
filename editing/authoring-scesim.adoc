...
As a developer of business decisions, you can use Test Scenarios in {PRODUCT} to test the decision logic in your _Business Services_ during development. Test Scenarios can be created and modified with {DEV_TOOLS}.

Some features present in {CANVAS} are not available for {DEV_TOOLS}, but you can achieve a similar experience of DMN Runner with Test Scenario Editor.

== Creating unit tests with Test Scenarios

Test Scenarios in {PRODUCT} enable you to validate the functionality of business decisions and business decision data. In a Test Scenario, you use data from your project to set given conditions and expected results based on one or more defined business decisions. When you run the scenario, the expected results and actual results of the decision instance are compared. If the expected results match the actual results, the test is successful. If the expected results do not match the actual results, then the test fails.

You define Test Scenarios in `.scesim` (scenario simulation) files that you can model using {DEV_TOOLS}. You can use one or multiple `.scesim` files in your _Business Service_ project, and each `.scesim` file can contain one or multiple Test Scenarios based on the defined decision data.
...
This documentation provides a step-by-step guide on how to use the Test Scenario Editor in VS Code to test DMN assets created in {PRODUCT_SHORT}. For more examples, see xref:../getting-started/examples.adoc[Example Business Service projects].

NOTE: {PRODUCT_SHORT} supports DMN-based Test Scenario only and not the Rule-based Test Scenario. It is still possible to **run** a Rule-based Test Scenario generated with {PRODUCT_SHORT} version {VERSION_80X} in Business Central using the old APIs.

// TBD move to upgrade

Also, all the DMN-based scesim assets created prior to version {VERSION} require a user action to update the related DMN file location. This is required because the DMN file path is now managed in a relative format.

== Project Setup

To run the scenario, you must have set up {MVN_REPO} and {DEV_TOOLS}. See xref:../installation/development-environment.adoc[Installing → Dev Environment]. Optionally, you can use Language Support for Java(TM) by Red Hat.

== Accessing the Example Project

If you are a new {PRODUCT_SHORT} user and do not have any project to set up yet, or you prefer having an out-of-the-box project to deal with, you can use an example project available in BAMOE.

To access the example project, go to the xref:../release-notes/whats-new-9-5.adoc#download-doc[Download documents] section, download the Example ZIP file and choose the example that matches your _Business Service_ project requirements.

The example project contains all the necessary files and prerequisites for this tutorial.

== Setting up any project

To enable Test Scenario in your project, follow these steps:

1. Add the `org.kie.kogito:kogito-scenario-simulation` dependency to your project's POM file to enable the Test Scenario runner. For the dependency, see xref:../reference-guide/maven-repository-libraries.adoc#_decisions_rules_and_decision_tables_dev_testing[Decisions, Rules, and Decision Tables (Dev + testing)].

2. Create the `KogitoScenarioJunitActivatorTest.java` file in the `src/test/java/testscenario` directory:
+
[source,java]
----
...
3. Create the `src/test/resources` directory for storing `.scesim` files.

== Creating a Test Scenario
Create the `PersonDecisions.dmn` file in the `src/main/resources` directory.
Its content is available in the xref:../getting-started/business-rule-microservice.adoc#example-person-decisions-dmn[Example PersonDecisions DMN file] section.
This is the first model that is tested using a `.scesim` file.
...
The corresponding `PersonDecisions.dmn` model in this example contains an `isAdult` DMN decision that determines that persons older than 18 years of age are adults. This example Test Scenario tests whether the person John Quark, aged 20, is correctly identified by the `isAdult` DMN decision as an adult (`true`), based on the decision logic.
...
This example scenario tests whether the person Jenny Quark, aged 15, is correctly identified by the `isAdult` DMN decision as not an adult (`false`), based on the decision logic.
--
...
image::test-scenarios/kogito-dmn-example-person.png[Image of PersonDecisions DRD,400]

.Example DMN boxed expression for 'isAdult' decision

image::test-scenarios/kogito-dmn-example-person-logic.png[Image of PersonDecisions Decision Table,480]
...
image::test-scenarios/kogito-test-scenario-example-person-background.png[Image of Test Scenario template,600]

.Example background definition with repeated value for 'Age'

image::test-scenarios/kogito-test-scenario-example-person-background2.png[Image of Test Scenario template,600]

.Modified Test Scenario template with excluded 'Age' column

image::test-scenarios/kogito-test-scenario-example-person-background3.png[Image of Test Scenario template,600]
...
image::test-scenarios/kogito-dmn-example-traffic-violation.png[Image of Traffic Violation decision diagram,600]

.Example DMN boxed expression for 'Fine' decision

image::test-scenarios/kogito-dmn-example-traffic-violation-fine.png[Image of Fine Decision Table,720]

.Example DMN boxed expression for 'Should the driver be suspended?' decision

image::test-scenarios/kogito-dmn-example-traffic-violation-suspension.png[Image of Should the driver be suspended? context expression,720]
