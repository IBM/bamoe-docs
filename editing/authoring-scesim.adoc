...
NOTE: At the current time, {PRODUCT_SHORT} supports DMN-based Test Scenario only. Rule-based Test Scenario are not supported yet. It's still possible to **run** a Rule-based Test Scenario generated with {PRODUCT_SHORT} {VERSION_80} in Business Central using the old APIs. For example, see xref:..editing/authoring-scesim.adoc[Example applications].
...
image::test-scenarios/kogito-test-scenario-example-person-create.png[Image of Create Test Scenario window]
