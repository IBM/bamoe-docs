...
You define test scenarios in `.scesim` (scenario simulation) files that you can model in Visual Studio Code (VS Code) using the {PRODUCT} https://marketplace.visualstudio.com/items?itemName=IBM.bamoe-developer-tools[VS Code extension]. You can use one or multiple `.scesim` files in your {PRODUCT} project, and each `.scesim` file can contain one or multiple test scenarios based on the defined decision data.
