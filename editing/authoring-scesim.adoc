...
As a developer of business decisions, you can use test scenarios in {PRODUCT} to test the decision logic in your {PRODUCT} decision services. Test scenarios enable you to ensure that your {PRODUCT} decision services are functioning properly and as expected as you develop your Business Service application.

Currently, Test Scenario models can be created and modified with {QUARKUS_DEV_UI}.

Static file validation, DMN Runner, and Dev Deployments are not available for {DEV_TOOLS} yet, but you can achieve a similar experience of DMN Runner with BAMOE's Test Scenario Editor.
See how below.
...
To run this scenario, you must have set up {MVN_REPO} and {DEV_TOOLS}. See xref:../installation/installation.adoc[Installing]. Optionally, you can use Language Support for Java(TM).
...
This example test scenario file tests the decision logic for the following `PersonDecisions.dmn` model in the _Business Service_ project. This DMN model determines whether a specified person is an adult or is underage.
...
NOTE: The following feature is available in the Classic Test Scenario Editor only. In the new Test Scenario Editor, you need to use a DMN FEEL expression to define a List value
...
.Procedure
. In your VS Code IDE, open the DMN file that contains the decisions that you want to test and select the *Data Types* tab to view the data types for that DMN model.
...
For more information about supported test scenario actions and data types, see the *Scenario Cheatsheet* icon in the right toolbar of the test scenario modeler in Visual Studio Code (VS Code).
