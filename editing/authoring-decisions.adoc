...
Below the DRDs, the preview shows boxed expressions from decision and BKM nodes. These expressions are non-editable and duplicate expressions are removed for clarity. Each node includes questions, allowed answers, and documentation links. The document also lists all data types with their corresponding type and constraints in a structured format.
