...
DMN models are decision models based on a notation standard defined by the Object Management Group (OMG). It uses graphical decision requirements diagrams (DRDs) that represent part or all of the overall decision requirements graph (DRG) to trace business decision flows, and uses an XML schema to enable sharing between standalone DMN editor platforms. The DMN Editor is available in {CANVAS}, and {DEV_TOOLS} and other standalone editors.

It is support Friendly Enough Expression Language (FEEL) to define decision logic in DMN Decision Tables and other DMN boxed expressions, and can be integrated efficiently with Business Process Model and Notation (BPMN) process models. They are optimal for creating comprehensive, illustrative, and stable decision flows.
...
This version of DMN editor is compatible with DMN 1.5. The 9.0.x editor remains available and is referred to as the DMN Editor (classic). By default, the DMN Editor will be opened when you first open {CANVAS}. 

Older DMN files can be opened directly in the DMN Editor; however, if they are edited and saved in this editor, they will no longer be compatible with the DMN Editor (classic).
...
This section describes how to use the DMN Editor.
...
+
The DMN Editor empty state enables you to easily bootstrap a new model via two options: by
...
In the empty state you can create a DMN model using one of the two options:
...
==== 8. Working with Multiple DRDs in the DMN Editor
...
`Collapsed Form`: 
In the Collapsed Form, the Decision Service is displayed as a single, compact element on the DRD canvas. This form emphasizes simplicity and abstraction, allowing modelers to reference the service without displaying its internal details.
The element’s user interface includes an expand button that, when clicked, reveals the full internal structure of the Decision Service. In its collapsed form, the internal decisions and components are hidden from view, allowing the diagram to remain clean and uncluttered. This is especially useful when the detailed logic of the service is not immediately relevant to the modeling task at hand.

.Collapsed Form
image::tools/authoringDecisions/collaps_form.png[width=300px]

`Expanded Form`:
In the Expanded Form, the internal structure of the Decision Service is fully visible, shown within a distinct boundary that visually encapsulates all internal elements. All internal logic and decision dependencies are presented in detail, providing full visibility into how the service operates.
A collapse button is available to return the Decision Service to its Collapsed Form when clicked. In the Expanded Form, the layout and structure of internal elements must remain consistent across all DRDs where the service is expanded. If the service is being expanded for the first time in any DRD, an auto-layout algorithm is used to generate a clean and organized visual structure. This expanded view is especially useful when a deeper understanding or modification of the service logic is required within a specific modeling context.

.Expanded Form
image::tools/authoringDecisions/expand_form.png[width=300px]

===== Expansion Rules and Conflict Management
To ensure clarity and avoid visual conflicts, the editor enforces the following rules:
*Exclusive Expansion: A Decision Service can only be expanded if none of its internal decisions are already expanded in another Decision Service on the same DRD.
*Conflict Handling: If a conflict exists, the expand button is disabled (greyed out) for the affected service.
*To resolve the conflict, collapse the currently expanded service that shares internal decisions.
These rules align with the DMN 1.5 specification and ensure a clean, non-overlapping diagram.

=====  Synchronized Layout Across DRDs
To maintain consistency:
*Position and size changes to decisions within a Decision Service are automatically synchronized across all DRDs.
*This ensures that the same Decision Service appears identically wherever it is used, reinforcing visual consistency and reducing modeling errors.
