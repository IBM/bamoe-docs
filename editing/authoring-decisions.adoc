...
For more information, see xref:#dmn-1.6-features
...
* Implement B-FEEL: The decision engine supports B-FEEL (Business Friendly Enough Expression Language) as an expression language. B-FEEL is a variant of FEEL that modifies the semantics to be more user-friendly.
* Simplify input data assignment across nested imports: You can assign the input data values only once and share them consistently across multiple models with nested imports, eliminating the need for redundant assignments and reducing model complexity.
* Boxed filter and boxed iterator behavior: The decision engine introduces a clarification to improve consistency and usability around boxed filter and boxed iterator expressions. Expressions not explicitly returning collections can now be processed correctly through implicit coercion into singleton lists.
* Apply clarified collect hit policy: You can use the Collect hit policy to return either all hits in arbitrary order or the result of applying a simple function to them, providing more control over the output of your decision tables.
* Migrate from Time type with Timezone to FEEL function: The decision engine deprecates the use of time type with timezone. Instead, use the FEEL function date and time (date, time, timezone) to construct a full date-time value.
* Utilize single-parameter number functions: The decision engine supports a single-parameter number() function that dynamically handles both string and number inputs, simplifying the use of number functions in your decision models.
* Streamline rounding functions with single-parameter versions: You can use the new single-parameter rounding functions, which assume a default scale of 0, to simplify the rounding of numbers and reduce model complexity.
* Automate type conversion from decimal to integer: The decision engine introduces a new type conversion rule that enables automatic conversion from decimal numbers to integers in integer-required contexts.
* Review of referencing items within the same DMN file: The decision engine explicitly permits either the namespace URI or the fragment identifier for referencing decisions within a single DMN file.
* Combine date, time, and timezone with new conversion function: You can utilize the new date and time (date, time, timezone) function to combine date, time, and timezone into a full date-time value, ensuring accuracy and consistency in your date and time calculations.
* Configure error handling modes: Choose between lenient and strict error handling modes to manage errors during decision model evaluation, providing control over the behavior of your decision models.
* Access new properties for date and time types: Leverage the new properties for date, date and time, years and months duration, and days and time duration types to access and manipulate date and time values in your decision models.
* Recursively access values with descendant operator: Use the new descendant operator to recursively access all values associated with a given property name, enhancing the expressiveness and flexibility of your decision models.