...
. Open the DMN editor and select *Generate Document* from the *Share* dropdown in the toolbar. This action launches a dialog interface that lists all DMN models in the current project.
. Select one or more DMNs to include in the document. The Generate button becomes active once you select at least one file.
. Click *Generate* to open a new browser tab with an HTML preview of the document. The preview displays visual representations of all DRDs for each selected DMN, arranged sequentially with their expression language and namespace.
