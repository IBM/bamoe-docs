...
For more information, see xref:../reference-guide/dmn.adoc#dmn-1.6[DMN 1.6 features].
...
== Generating document for multiple DMN models in BAMOE

IBM BAMOE introduces a document generation feature that allows you to create a unified HTML or PDF document containing multiple DMN models from Canvas. This enhancement improves usability for projects with complex decision models composed of several DMN files. 

.Procedure
To generate unified documentation

. open the DMN editor and select *Generate Document* from the Share dropdown in the toolbar. This action launches a modal interface that lists all DMN models in the current project.
. Use the search option to quickly locate models and select one or more DMNs to include in the document. The Generate button becomes active once you select at least one file.
. Click Generate to open a new browser tab with an HTML preview of the document. The preview displays visual representations of all DRDs for each selected DMN, arranged sequentially with their expression language and namespace. You can zoom in and out to explore diagrams. Below the DRDs, the preview shows boxed expressions from decision and BKM nodes. These expressions are non-editable and deduplicated for clarity. Each node includes questions, allowed answers, and documentation links. The document also lists all data types with their corresponding type and constraints in a structured format.
. To export the document, use the *Print* button located at the top-left corner of the preview page. This button invokes the browser’s native window.print() functionality, enabling you to print the document or save it as a PDF. The generated document will include DMN diagrams, boxed expressions, and data types, compiled from the files you select. Each selected file contributes its content to the final documentation.
