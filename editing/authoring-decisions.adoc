...
==== version 1.6
* Support for B-FEEL: The decision engine now supports B-FEEL (Business Friendly Enough Expression Language) as an expression language. B-FEEL is a variant of FEEL that modifies the semantics to be more user-friendly.
* Simplified InputData Assignment: The decision engine simplifies how InputData values are assigned across models with nested or transitive imports. InputData values are now assigned only once, and the value is automatically propagated across all import paths.
...
* Collect Hit Policy: The decision engine introduces a clarification to the Collect hit policy behavior. The Collect hit policy now returns either all hits in arbitrary order or the result of applying a simple function to them.
* Deprecation of Time Type with Timezone: The decision engine deprecates the use of time type with timezone. Instead, use the FEEL function date and time(date, time, timezone) to construct a full date-time value.
* Support for Single-Parameter Number() Function: The decision engine supports a single-parameter number() function that dynamically handles both string and number inputs.
* Support for Single-Parameter Rounding Functions: The decision engine introduces single-parameter versions of all rounding functions in FEEL. These new variants simplify usage by assuming a default scale of 0.
* Type Conversion from Decimal to Integer: The decision engine introduces a new type conversion rule that enables automatic conversion from decimal numbers to integers in integer-required contexts.
* Review of Referencing Items within the Same DMN File: The decision engine explicitly permits two distinct styles for referencing decisions within a single DMN file.
* Support for New Version of date and time Conversion Function: The decision engine supports a new conversion function date and time(date, time, timezone) that constructs a full date-time value by combining the provided date and time, and applying the specified timezone.
* Configurable Error Handling Modes: The decision engine introduces two distinct modes for managing errors during decision model evaluation: lenient and strict.
* Support for New Property “value” for Types: The decision engine defines a new value property for types date, date and time, years and months duration, and days and time duration.
* New Descendant Operator: The decision engine introduces a new descendant operator that enhances the expressiveness of FEEL when working with nested contexts.
