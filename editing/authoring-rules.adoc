...
DRL Rules are a flexible way of defining rules and rule behavior. They are the individual rules that you can define directly in `.drl` text files. They can be created in any standalone environments or text editor and integrated with {PRODUCT}. They Have strict syntax requirements for rules to be compiled properly.

== Structure of DRL Rules 

A Drools rule consists of three main parts:

 * Rule Declaration: This is where you give your rule a name and optionally specify metadata, like salience or agenda groups.
 * When: This section contains the conditions or constraints, specifying when the rule should be executed. It is similar to an if statement in programming language.
 * Then: This is the action section, where you define what happens when the conditions are met.
...
In the example, `Underage` is the rule name. `Salience`` is a priority value for rules, where a higher value indicates higher priority and the `agenda-group` enables you to group rules together and execute them based on priority. For more information on attributes, see https://docs.drools.org/8.39.0.Final/drools-docs/docs-website/drools/language-reference/index.html#ref-rules-attributes_drl-rules[rule attributes].

In the rule, the condition checks if the `Applicant` age is less than 21. If the condition set as 'Approved' is false, the action states that the applicant is `Underage`.
...
Ruleflow groups are a mechanism to control the execution order of rules. They allow you to group rules together and specify the order in which these groups must fire. Rules within the same group compete for execution, and the salience attribute within a rule determines their priority within the group. Ruleflow groups do not provide a namespace for rules, meaning rule names must be unique across the entire rule base.

.Rule unit

Rule units that are available since Drools version 10, are modules for rules that provide a unique namespace for each group of rules. A rule unit collects a set of rules along with the declaration of the type of facts that the rules act on. This means that rules within a rule unit do not compete with rules in other units, and rule names only need to be unique within the rule unit. Rule units can be seen as a way to encapsulate and modularize sets of related rules. Note that constraints in the rules that are in a Rule unit, must be expressed in a different way. See https://kie.apache.org/docs/10.1.x/drools/drools/language-reference/index.html#new-and-traditional-syntax_drl-rules[Drools Documentation]. 

In summary, while ruleflow groups help manage the execution order of rules, rule units provide a namespace for organizing and grouping rules, preventing naming conflicts across different sets of rules.
...
You can upload spreadsheet Decision Tables into {CANVAS}. These tables support template keys and values for creating rule templates. The decision tables are also helpful for creating rules in decision tables already managed outside of {CANVAS}. They have strict syntax requirements for rules to be compiled properly when uploaded.
...
.Prerequisites
* You have the `org.drools:drools-decisiontables` dependency added to the POM file of your _Business Service_ project to enable decision tables for decision services. For the dependency, see xref:../reference-guide/maven-repository-libraries.adoc#_decisions_rules_and_decision_tables_embedded_java_mode[Decisions, Rules, and Decision Tables (Embedded Java mode)].
...
+
* *Rule elements:* Conditions, actions, or rule attributes.
...
== Specifying rule attributes in a decision table
