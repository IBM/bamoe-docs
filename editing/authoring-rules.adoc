...
rule "rule name"
    // Attributes 
    https://docs.drools.org/8.39.0.Final/drools-docs/docs-website/drools/language-reference/index.html[Click this link for information on attributes]
    when
        // Conditions
    then
        // Actions
...
In the example, `Underage` is the rule name. `Salience`` is a priority value for rules, where a higher value indicates higher priority and the `agenda-group` enables you to group rules together and execute them based on priority.
...
+
* *Rule attributes:* Conditions, actions, or other attributes. https://docs.drools.org/8.39.0.Final/drools-docs/docs-website/drools/language-reference/index.html[Click this link for more information on attributes].
