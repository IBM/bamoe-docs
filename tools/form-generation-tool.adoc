...
This is a functionality of the {VS_CODE_BUNDLE} and is used to generate web forms for {PRODUCT_SHORT} and Apache Kie projects needed for the execution of processes. 
...
You can generate forms by using the existing {PRODUCT_SHORT} project, *process-compact-architecture-example* available at https://www.ibm.com/support/pages/node/7151078[https://www.ibm.com/support/pages/node/7151078]. You will find more example projects in this zip file, however *process-compact-architecture-example* requires the minimal effort to see forms in the action or you can use your own project.
...
When you select `BAMOE Developer Tools: Generate form code for User Tasks`, you must enter the absolute path of the {PRODUCT_SHORT} project. By default, this field is auto-filled with the currently opened workspace.
...
After entering the absolute path of {PRODUCT_SHORT} project, you must choose between `Bootstrap 4` and `PatternFly`, the two supported UI libraries. `Bootstrap 4` generates HTML code, while `PatternFly` generates React code.
...
== Form DevUI
For Quarkus-based {PRODUCT_SHORT} projects, you can use and test them by using the *jBPM Quarkus DevUI* extension.
...
Open the form and change the `Approve' to `I approve this candidate!`.
...
image::tools/form-gen-new-change.png[New interview]
...
[NOTE]
====
If the form contains Lists, all `badge` classes must be removed, and 'loat-right' should be replaced with 'float-end'.
====