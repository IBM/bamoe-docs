...
Once the prerequisites are met, it is easy to install the *{FORM_GENERATION_TOOL}*.
...
To install this package, simply issue the `npm` command below or any other command you use for installing `npm` packages:
...
NOTE: Please use a version later than *0.32.0*.


Once the installation is done, you can start using the *{FORM_GENERATION_TOOL}* from your command line.
Type the command below to ensure installation was successful.
...
We have only three Workflows in the `process-quarkus-example` project. 
User Tasks also have forms generated.
Even though there are only three Workflows in the process-quarkus-example project, there are more than three *.html* files due to the User Task generated forms.
...
[NOTE]
====
Currently there is an https://github.com/apache/incubator-kie-issues/issues/836[issue,window=_blank] blocking users from navigating to the *Forms* view. For more information, see xref:../introduction/release-notes/known-limitations.adoc[Known limitations].
====
