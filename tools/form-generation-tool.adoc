...
This is a functionality of the {VS_CODE_BUNDLE} and is used to generate web forms for {PRODUCT_SHORT} and Apache Kie projects needed for the execution of processes. Form Generation is included as a command in the BAMOE Developer Tools extension for VS Code (BAMOE VS Code extension).
...
You can generate forms by using an existing {PRODUCT_SHORT} project, *process-compact-architecture-example* available at xref:../getting-started/examples.adoc[Example applications].
