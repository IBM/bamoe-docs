...
Before you start using *{FORM_GENERATION_TOOL}*, please be sure you understand xref:../tools/authoring-workflows.adoc[Workflows], formerly known as Business Processes, properly.
...
- Node 18 (To install it, follow these https://nodejs.org/en/download/package-manager/[instructions])


Once the prerequisites are met, it is super simple to install *{FORM_GENERATION_TOOL}*.
It is available as `@kie-tools/form-generation-tool` `npm` https://www.npmjs.com/package/@kie-tools/form-generation-tool[package].
...
[NOTE]
Please use version higher than *0.32.0*.


Once the installation is done, you can start using *{FORM_GENERATION_TOOL}* from your command line. Simply type command below to double-check successful installation.
...
However, for this documentation we will use existing https://incubator.apache.org/projects/kie.html[Apache Kie] community projects stored https://github.com/apache/incubator-kie-kogito-examples[here], especially those that have a process in the project name, e.g. `process-quarkus-example`. Or you can use your own project.


Browsing linked examples, you may notice the projects use Maven. So when Maven builds an https://incubator.apache.org/projects/kie.html[Apache Kie] project, Kie generates JSON schemas to represent the data models for both Workflows and User Tasks.
...
- Make sure your https://incubator.apache.org/projects/kie.html[Apache Kie] project has been compiled, otherwise the CLI won't be able to find all the form schemas.
...
First, set the path to your https://incubator.apache.org/projects/kie.html[Apache Kie] Project. We will use https://github.com/apache/incubator-kie-kogito-examples/tree/stable/kogito-quarkus-examples/process-quarkus-example[process-quarkus-example].
...
One question that may come to your mind, is why there are so many forms. We have only three Workflows in `process-quarkus-example` project. 
Well spotted. It is because not only Workflows can have their forms, but also User Tasks, as special nodes that require input from users, can have their own forms.

== Form UI
For Quarkus based https://incubator.apache.org/projects/kie.html[Apache Kie] projects, you can use and test them by using the *jBPM Quarkus DevUI - Deployment* extension.
...
[NOTE]
Please use version *Apache Kie 10* or higher.
...
Then in your browser, please navigate to:
[source,bash]
----
...
[NOTE]
Currently there is an https://github.com/apache/incubator-kie-issues/issues/836[issue]. As a workaround, please use a url `http://localhost:8080/q/dev-v1/`.

On the displayed page, find there this card and click *Forms*.
...
Now we should see see a list of all available forms that we listed already via the commadn line earlier in this documentation.
