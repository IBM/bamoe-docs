...
All Git repositories in {CANVAS} are stored in-browser and can be synced with a remote Git repository by using a simplified set of operations, steering clear of the complex choices you have to deal with when using Git from the Command Line.

{CANVAS} auto-saves all changes you make and persists them on your disk.
You can safely close your browser tabs and turn off your computer.
Your work will be there when you come back. That being said we recommend that you Push your work after you're done for the development session, as should something happen to your computer, your changes will be safely stored with your Git provider.

Each card that appears on the Home page is a separate Git repository and can contain any number of files, nested directories, and file types.
...
If you want to work on two separate branches at the same time, you can import your repository again, selecting a different branch.

== Conflict resolution

Although rare if you're working on a feature branch, conflicts can still happen if you make changes externally or if you're sharing a branch with a peer.
