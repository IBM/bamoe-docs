...
== Reusing logic
For solving a problem in any domain, it is always helpfull to be able reuse some existing logic.
To make some logic reusable in the DMN, we use BKM or Decision Service nodes.

=== Reusing logic - Decision Service
We will take a closer look on the Decision Service node.
Using this node we will define once a logic, that can be reused unlimitedly in our DMN models or models that will import our Decision Service node.

The Decision Service node allows wrapping and structuring a larger set of Decision nodes, while producing a single output.
We will not start from the scracth, rather we will reuse and existing community example https://raw.githubusercontent.com/apache/incubator-kie-kogito-examples/8123b27b7fb64d6a0a4c4561d97487e2b6253cf7/kogito-quarkus-examples/onboarding-example/payroll/src/main/resources/org/kie/kogito/examples/payroll/taxRate.dmn[taxRate.dmn].
...
Using only single **encapsulated** node `taxRate` and single **output** node `compute Payroll` does not demonstrate the Decision Service node power enough.
We will elaborate this example a little bit.
...
We can also change model name from `taxes` to `payroll`.

image::tools/runningDecisions/reusing-logic-checkpoint-1.png[]

Now, lets make the Decision Service node `rate` bigger as we will add more content to it.
We will introduce a logic similar to `taxRate` node for computing vacation days and payment date.
...
We sucessfuly changed the structure of the logic, as we moved all Decision nodes into the **output** section, the Decision Service node will return Complex data, not Simple data.

NOTE: See xref:adding-data-type[] to learn more about data types.

Now, we need to define logic for the new nodes.
Set the `vacationDays` and `paymentDate` data type as `number`.
...
It is a good opportunity to use copy and paste feature we have.
This feature is available after click to the top left corner of the node expression.
...
NOTE: If you decided to use copy and paste feature, you will need to rename nodes back to `vacationDays` and `paymentDate`.

Lets finish the new decision tables.
Lets return **38** vacation days for **"US"** and **"33"** vacation days for **"UK"**.
Lets return **15** as a payment date for **"US"** and **"10"** as a payment date for **"UK"**.
...
NOTE: See xref:adding-data-type[] to learn more Complex data types.
...
For the Invocation we need to specify function name as `payroll` and the single parameter `employee` with passed value `employee`.
In this case, the `compute Payroll` Data type will be `tPayroll`.

NOTE: The complete list of paramaters that Decision Service node requires to be invoked can be also seen in the Properties panel **Invoking this Decision Service in FEEL** section.
...
NOTE: See xref:running-decisions.adoc[DMN Runner page] to learn more details.

There is availavle source code of the https://raw.githubusercontent.com/bamoe/bamoe-docs-source-code-examples/9.1.x/dmn/payroll.dmn[payroll.dmn] example model.
