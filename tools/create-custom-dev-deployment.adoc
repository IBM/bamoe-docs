...
In other words, after the xref:../reference-guide/deployment_tokens.adoc#token_interpolation[token interpolation] is finished, the resulting command executed on the container will be `dev-deployment-upload-service && ./mvnw quarkus:dev`.
