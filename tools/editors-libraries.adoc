...
Since version {VERSION_911} the BPMN and DMN Editors used in both {CANVAS} and {DEV_TOOLS} are also available as standalone JavaScript libraries that can be embedded into any frontend web application.

They are available by importing the following libraries from NPM:

* {STANDALONE_DMN_EDITOR_LINK}[**@ibm/bamoe-standalone-bpmn-editor**]: The new and modern BPMN Editor (Tech Preview) - _Since 9.2.1_
* {STANDALONE_BPMN_EDITOR_LINK}[**@ibm/bamoe-standalone-dmn-editor**]: The new and modern DMN Editor
* {STANDALONE_EDITORS_CLASSIC_LINK}[**@ibm/bamoe-standalone-bpmn-and-dmn-editors-classic**]: The BPMN and DMN Editors (classic)

== BPMN Editor (Tech Preview)

The modern BPMN Editor, built with React, is faster and aims to be retro-compatible with BPMN Editor (classic), supporting the BPMN 2.0 specifications while maintaining the same simple-to-use API.

.BPMN Editor
image::tools/editorsLibraries/new-bpmn-editor.jpg[]

=== Installing and using the BPMN Editor

Follow these steps to install the DMN Editor

. Add the library to the project's `package.json`:
+
[source,shell,subs="+attributes"]
----
npm install @ibm/bamoe-standalone-bpmn-editor@{NPM_VERSION}
----
...
NOTE: The `initialFileNormalizedPosixPathRelativeToTheWorkspaceRoot` parameter is only available for the BPMN and DMN Editors. BPMN and DMN Editors (classic) do not have the concept of relative paths, so every mapped resource is available.
...
NOTE: As of now, the BPMN Editor does not expose the `canvas` property.
...
[source,shell,subs="+attributes"]
----
npm install @ibm/bamoe-standalone-dmn-editor@{NPM_VERSION}
----
...
NOTE: The `initialFileNormalizedPosixPathRelativeToTheWorkspaceRoot` parameter is only available for the DMN Editor. The BPMN and DMN Editors (classic) do not have the concept of relative paths, so every mapped resource is available.
...
NOTE: As of now, the DMN Editor does not expose the `canvas` property.
