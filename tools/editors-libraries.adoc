...
`getPreview(): Promise<string>`:: Returns a Promise that contains the SVG string of the current diagram.

`subscribeToContentChanges(callback: (isDirty: boolean) => void): (isDirty: boolean) => void`:: Sets and retuns a callback that needs to be called on every content change in the Editor. The same callback is used for unsubscription.
...
|`initialContent` | Assigns a Promise to the XML content of a DMN file for the DMN Editor. It can be empty.  | Mandatory   |
* `Promise.resolve("")`
