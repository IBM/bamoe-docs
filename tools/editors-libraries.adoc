...
Then load each Editor, passing the reference to the respective `div` container:
...
<2> `initialContent`: Promise to a BPMN XML content for the BPMN Editor or the XML content of a DMN file for the DMN Editor. It can be empty. Following are the examples:
 * `Promise.resolve("")`
 * `Promise.resolve("<BPMN_OR_DMN_CONTENT_DIRECTLY_HERE>")`
 * `fetch("MyBpmnFile.bpmn").then(content => content.text())`
<3> `readOnly` (optional, defaults to `true`): Use `false` to allow content edition and `true` for read-only mode, where the editor will not allow changes.
...
* `subscribeToContentChanges(callback: (isDirty: boolean) => void): (isDirty: boolean) => void`: Setup a callback to be called on every content change in the Editor. It returns the same callback to be used for unsubscription.
