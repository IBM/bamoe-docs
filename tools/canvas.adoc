...
You can change the {CANVAS} interface language either by updating your browser’s language settings or by using the language dropdown in Canvas. In Canvas, navigate to Settings, open the Language tab, and select your preferred language from the dropdown. Once configured, all UI elements, including the {CANVAS} homepage and interactive components, will display in the selected language, providing a fully localized experience.