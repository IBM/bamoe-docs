...
image::tools/newProject/canvas-create-project.png['Create section at the Home page of {CANVAS}',600]
...
. *Problems tab* → Lets you see what is wrong with your file, both statically and when running it.
...
You can do that by creating another file using the “New file menu”. You can see that there are new controls available on the screen.
You can look at them and see what they do.
...
. *Repository status indicator* → Tells you the status of your repository.
  ...
  To make sure you are synced, you need to do a Sync operation.
...
Now that you are familiar with the elements of {CANVAS}, you can jump to xref:../editing/authoring-decisions.adoc[Authoring Decisions with DMN] or xref:../workflow/authoring-workflows.adoc[Authoring Workflows with BPMN] to start authoring Decisions and Workflows to create a Business Automation project. xref:../tools/applying-accelerators.adoc[Using Accelerators] can also be interesting if you are starting a new project directly from {CANVAS}.
