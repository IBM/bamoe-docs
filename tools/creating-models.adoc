...
[role="_abstract"]
{PRODUCT} provides the following extensions or applications that you can use to design Business Process Model and Notation (BPMN) process models and Decision Model and Notation (DMN) decision models for your _Business Services_ using graphical modelers.

* *{CANVAS}*: Enables you to work with Decisions and Workflows, directly in the browser. See the xref:../tools/canvas.html[{CANVAS}] chapter for further details.

* {DEV_TOOLS_VSCODE_MARKETPLACE_LINK}[*{DEV_TOOLS}*]: Enables you to view and design Decisions (`.dmn`), Workflows (`.bpmn`), and Test Scenario (`.scesim`) models in Visual Studio Code (VS Code). See the xref:../tools/developer-tools-for-vscode.adoc[{DEV_TOOLS}] chapter for further details.
