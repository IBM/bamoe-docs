...
NOTE: For GitHub Enterprise Server and GitLab, use a Personal Access Token. For Bitbucket Data Center, use a Username and Personal Access Token. For more information on generating Personal Access Tokens, see xref:https://docs.gitlab.com/user/profile/personal_access_tokens/[GitLab Docs] and xref:https://confluence.atlassian.com/bitbucketserver/http-access-tokens-939515499.html[Bitbucket Support].
...
.Code Snippets
You can create two types of snippets in GitLab- Personal Snippets, which belong to a user, and Project Snippets, which are associated with a project. You can choose to create these snippets under their own account or under a specific project. The visibility settings for these snippets are inherited from the project they belong to. Additionally, since snippets are not full Git repositories, they do not support branching like projects do.
...
NOTE: Bitbucket Data Center does not support native code snippets, so snippet-related features are not available.
