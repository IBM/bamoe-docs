...
+
NOTE: The Test Scenario Editor (classic) should be considered a deprecated tool. That means, no new feature will land in this editor version and it will be eventually removed in a future {PRODUCT_SHORT} version. This editor parses DMN assets generated with the DMN Editor (classic) only (DMN version 1.2).

The new Editor is currently set as the default choice for scesim assets. The steps to switch to the Test Scenario Editor (classic) are:
...
The scesim files produced by the new Test Scenario Editor can be consumed by the Test Scenario Editor (classic).
