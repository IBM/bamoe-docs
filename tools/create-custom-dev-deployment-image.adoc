...
Custom Dev Deployment options via user-defined YAMLs provide complete control over how your business automation models are deployed to Kubernetes or OpenShift clusters. Instead of relying on the default Quarkus Blank App or Custom Image options, you can define deployment configurations that precisely match your infrastructure requirements, organizational standards, and deployment scenarios.

To create custom Dev Deployment options, you define deployment configurations using standard Kubernetes YAML manifests, JSON Patches, and descriptor files. This process involves creating a specific directory structure within your project or accelerator, defining the Kubernetes resources your deployment requires, and configuring parameters that users can adjust at deployment time.
...
* *manifests/*: Contains standard Kubernetes resource YAML files.
...
=== Dev Deployment Option via user-defined YAMLs in a Business Service project
...
{CANVAS} validates `option.json` files against a JSON schema derived from `TypeScript` type definitions. {CANVAS} displays validation errors in the Deploy dialog with specific error messages. If there are errors, then you must revisit the deployment option configuration and correct it before reopening the dev deployment modal.
