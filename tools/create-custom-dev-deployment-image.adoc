...
When calling Maven, `./mvnw` is used by default since this will use the wrapped version of Maven from your project. However, the Maven binary is available globally in this image, and it's possible to call `mvn` directly.
