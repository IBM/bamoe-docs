...
This is a functionality of the {VS_CODE_BUNDLE} and is used to generate web forms for {PRODUCT_SHORT} and Apache KIE projects. Form Generation is included as a command in the BAMOE Developer Tools extension for VS Code (BAMOE VS Code extension).
...
This functionality is included as a command in the BAMOE Developer Tools extension for VS Code (BAMOE VS Code extension). To run a command in VS Code, open the command palette using the keyboard shortcut `CTRL + SHIFT + P` for Windows and Linux or `CMD + SHIFT + P` for macOS.
...
After entering the absolute path of {PRODUCT_SHORT} project, you must choose between `Bootstrap 4` and `PatternFly`, the two supported UI libraries.
...
<dependency>
    ...
    <version>9.2.0-ibm-0003</version>
</dependency>
----
