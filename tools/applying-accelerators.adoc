...
Successfully applying an Accelerator creates a `.kie-sandbox` directory on your workspace root. This directory will have an `accelerator.yaml` file which contains all information used during the apply operation. The file indicates that an Accelerator was applied to the workspace and, therefore mustn't be removed. Clicking on the Accelerator label will open a modal with the `accelerator.yaml` information.
