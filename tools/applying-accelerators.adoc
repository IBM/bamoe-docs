...
Accelerators are project templates that can be applied to existing folders containing Workflows (BPMN), Decisions (DMN), and Rules (DRL), transforming them into complete _Business Service_ projects that can be built, tested, executed, and deployed with your preferred development framework (Quarkus or Spring Boot).

== From existing Decision and Workflow models

{CANVAS} provides a set of useful default Accelerators:
