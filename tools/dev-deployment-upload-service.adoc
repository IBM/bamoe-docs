...
To install it, download the zip file, extract it, and copy the correct executable somewhere in your `PATH` (usually `/usr/local/bin`).
...
_(`8080` on the Canvas Dev Deployment Base image)_
...
NOTE: The Dev Deployment Upload Service requires your system to have both the `tar` and `gzip` libraries installed.