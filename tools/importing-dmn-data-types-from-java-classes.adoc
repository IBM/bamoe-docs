...
{DEV_TOOLS} allows you to create DMN Data Types directly from Java POJOs. The following section describes an example scenario to demonstrate how to do this in the new DMN editor.
...
* **Select Java Classes:** In the search box, specify the name of the Java Class you want to import as a DMN Type. 

+
image::developer-tools/dmn-import-java-classes-specify.png[]
+

You can use the class simple name (eg. Book) or the full qualified name (eg. dto.Book). In the search box, a list with all Java Classes that match the provided key will appear. Select the ones you want to import. When ready, click on the `Next` button.
+
