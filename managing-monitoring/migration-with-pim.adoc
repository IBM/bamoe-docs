...
NOTE: If you are upgrading from {VERSION_930} ensure you have run the database migration scripts (see xref:.https://www.ibm.com/docs/en/ibamoe/9.4.x?topic=upgrading-from-93x[Upgrading your stateful Workflow database]) to add required PIM tables (see xref:../reference-guide/stateful-workflows-database-tables.adoc#ppim-db-tables[Reference → Stateful Workflows subsystem db tables])
...
If you are using Db2 the batch size recommendation is to migrate up to 32,000 Process Instances at any one time. 
