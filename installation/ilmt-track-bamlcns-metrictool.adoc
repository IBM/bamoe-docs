...
License Metric Tool automatically detects the following {PRODUCT_LONG} {VERSION90X} release chargeable components that are part of the product:
...
To detect and count remote nodes that are managed by {PRODUCT_LONG} chargeable components, you must manually deploy software tags because no product code is present on those nodes. 
License Metric Tool generates and assigns dedicated software tags during the creation of the Readiness Package for the latest release of {PRODUCT_LONG}.

The following {PRODUCT_LONG} chargeable components require manual deployment of software tags, for example {VERSION901}:
...
NOTE: The {PRODUCT_LONG} 9.0 license can be used for all the {PRODUCT_LONG} access methods and application plug-ins available on the Automation Hub and its {VERSION90X} fix releases.
