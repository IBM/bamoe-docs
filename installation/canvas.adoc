...
* BAMOE Extended Services: provides features like DMN Runner, static validation of DMN, BPMN models, and a proxy to communicate with OpenShift and Kubernetes clusters.
* BAMOE CORS Proxy: provides a proxy to allow {CANVAS} web app to communicate with Git providers, like github.com and bitbucket.org.
...
* BAMOE Canvas → https://quay.io/repository/bamoe/canvas[https://quay.io/repository/bamoe/canvas] @ {IMAGE_VERSION}
* BAMOE Extended Services → https://quay.io/repository/bamoe/extended-services[https://quay.io/repository/bamoe/extended-services] @ {IMAGE_VERSION}
* BAMOE CORS Proxy → https://quay.io/repository/bamoe/cors-proxy[https://quay.io/repository/bamoe/cors-proxy] @ {IMAGE_VERSION}
...
----
. Deploy BAMOE Extended Services and label its resources
+
[source,shell,subs="attributes+"]
----
...
----
. Deploy BAMOE CORS Proxy and label its resources
+
[source,shell,subs="attributes+"]
----
...
----
. Finally, deploy the {CANVAS} image, setting the environment variables required to connect it to the BAMOE Extended Services and BAMOE CORS Proxy backends.
+
[source,shell,subs="attributes+"]
----
...
NOTE: All services will be available via `https`, which means that if you're deploying them to a local OpenShift instance (i.e. via CRC), the {CANVAS} web app, the BAMOE Extended Services and BAMOE CORS Proxy backends will all be using self-signed certificates, causing warnings in your browser. To fix this, access each URL individually and allow them in your browser.
