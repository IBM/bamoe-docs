...
* Git CORS Proxy → https://quay.io/repository/bamoe/git-cors-proxy[https://quay.io/repository/bamoe/git-cors-proxy] @ {VERSION}
...
oc new-app quay.io/bamoe/canvas:{VERSION} --name=$APP_NAME_BAMOE_CANVAS \
  ...
  -e KIE_SANDBOX_CORS_PROXY_URL=https://$(oc get route $APP_NAME_GIT_CORS_PROXY --output jsonpath={.spec.host})
...
`KIE_SANDBOX_CORS_PROXY_URL`:: E.g., https://git-cors-proxy.bamoe.company.com[https://git-cors-proxy.bamoe.my-company.com]
