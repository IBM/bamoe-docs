...
----
NOTE: By default all routes are deployed with SLL enabled. Use `https://` for every host when checking them out.
...
# Add the -d flag to run in detached mode.
----
The API is accessible via port 8090 (8080 is reserved for the bamo-canvas image), no additional configuration is necessary.
...
{CANVAS} can also be deployed via Docker Compose or Helm Charts, check them out on the pages below:
