...
This Chart uses default environmental variables from `values.yaml` file. You can override the variables by passing them through the command line. See xref:../installation/canvas.adoc#helm-chart-config-ref[{CANVAS} Helm Chart configuration reference] for all options.
...
For other customization options of {CANVAS}, please refer to the https://quay.io/repository/bamoe/canvas?tab=info[{CANVAS}'s image description on Quay.io].
