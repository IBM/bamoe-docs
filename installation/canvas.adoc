...
* Canvas → https://quay.io/repository/bamoe/canvas[https://quay.io/repository/bamoe/canvas] @ 9.1.0-ibm-0001
* Extended Services → https://quay.io/repository/bamoe/extended-services[https://quay.io/repository/bamoe/extended-services] @ 9.1.0-ibm-0001
* CORS Proxy → https://quay.io/repository/bamoe/cors-proxy[https://quay.io/repository/bamoe/cors-proxy] @ 9.1.0-ibm-0001
...
[source,shell,subs="attributes+"]
----
oc new-app quay.io/bamoe/extended-services:{VERSION_910} --name=$APP_NAME_EXTENDED_SERVICES
...
[source,shell,subs="attributes+"]
----
oc new-app quay.io/bamoe/cors-proxy:{VERSION_910} --name=$APP_NAME_CORS_PROXY
...
[source,shell,subs="attributes+"]
----
oc new-app quay.io/bamoe/canvas:{VERSION_910} --name=$APP_NAME_BAMOE_CANVAS \
...
[source,shell,subs="attributes+"]
----
docker run -p 21345:21345 -it quay.io/bamoe/extended-services:{VERSION_910}
...
[source,shell,subs="attributes+"]
----
docker run -p 8090:8080 -it quay.io/bamoe/cors-proxy:{VERSION_910}
# Add the -d flag to run in detached mode.
----
...
If you're running locally, following the previous steps, use this command to start your {CANVAS} instance with the backend services:
[source,shell,subs="attributes+"]
----
docker run -p 8080:8080 -it \
  -e KIE_SANDBOX_EXTENDED_SERVICES_URL=http://localhost:21345 \
  -e KIE_SANDBOX_CORS_PROXY_URL=http://localhost:8090 \
  quay.io/bamoe/canvas:{VERSION_910}
# Add the -d flag to run in detached mode.
----
The HTTP server is accessed via port 8080.

Several customization options are available. Check out more configurations available via environment variables here: https://quay.io/repository/bamoe/canvas?tab=info[https://quay.io/repository/bamoe/canvas?tab=info]
...
If you're installing {CANVAS} to an air-gapped environment, you're going to need to configure `KIE_SANDBOX_DMN_DEV_DEPLOYMENT_BASE_IMAGE_URL` to point to an image in a registry where the Cloud providers used by {CANVAS} users can access it.
