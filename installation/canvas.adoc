...
To integrate GitHub Enterprise Server, GitLab, or Bitbucket Data Center with BAMOE Canvas, you must configure the `KIE_SANDBOX_AUTH_PROVIDERS` environment variable.

NOTE: You can add environment variable configuration to your docker-compose.yml file or pass it directly via a docker run command. For more information, see https://quay.io/repository/bamoe/canvas[BAMOE Canvas image description on Quay.io] .
...
* disableEncoding: If set to true, adds Accept-Encoding (specific for GitLab).
* insecurelyDisableTlsCertificateValidation: If set to true, skips TLS certificate validation (specific for BitBucket Data Center).
