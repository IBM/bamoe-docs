...
[source,shell]
----
oc new-app quay.io/bamoe/extended-services:9.0.0 --name=$APP_NAME_EXTENDED_SERVICES
...
[source,shell]
----
oc new-app quay.io/bamoe/git-cors-proxy:9.0.0 --name=$APP_NAME_GIT_CORS_PROXY
...
[source,shell]
----
oc new-app quay.io/bamoe/canvas:9.0.0 --name=$APP_NAME_BAMOE_CANVAS \
...
NOTE: All services will be available via `https`, which means that if you're deploying them in a local OpenShift instance (i.e. via CRC), the {CANVAS} web app, the Extended Services and Git CORS Proxy backends will all be using self-signed certificates, causing warnings in your browser. To fix this, access each URL individually and allow them in your browser.
...
[source,shell]
----
docker run -p 21345:21345 -it quay.io/bamoe/extended-services:9.0.0
...
[source,shell]
----
docker run -p 8090:8080 -it quay.io/bamoe/git-cors-proxy:9.0.0
# Add the -d flag to run in detached mode.
----
...
[source,shell]
----
docker run -p 8080:8080 -it quay.io/bamoe/canvas:9.0.0 \
...
{CANVAS} can also be deployed via Docker Compose or Helm Charts (to a Kubernetes cluster, for example), check them out on the pages below:

* xref:installation/docker-compose.adoc[Docker Compose]
* xref:installation/helm-charts.adoc[Helm Charts]