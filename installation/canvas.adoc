...
NOTE: All services will be available via `https`, which means that if you're deploying them to a local OpenShift instance (that is, via CRC), the {CANVAS} web app, the Extended Services and Git CORS Proxy backends will all be using self-signed certificates, causing warnings in your browser. To fix this, access each URL individually and allow them in your browser.
...
`KIE_SANDBOX_GIT_CORS_PROXY_URL`:: For example, https://git-cors-proxy.bamoe.company.com[https://git-cors-proxy.bamoe.my-company.com]
