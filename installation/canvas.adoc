...
NOTE: All services will be available via `https`, which means that if you are deploying them to a local OpenShift instance (i.e. via CRC), the {CANVAS} web app, the Extended Services and CORS Proxy backends will all be using self-signed certificates, causing warnings in your browser. To fix this, access each URL individually and allow them in your browser.
