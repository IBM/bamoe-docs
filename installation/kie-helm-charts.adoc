...
BAMOE Canvas helm chart is a Helm chart based on the opensource KIE Sandbox and it is a technology that can enable you to install the multi-container services with a single command: Helm, https://helm.sh/[the package manager for kubernetes].
