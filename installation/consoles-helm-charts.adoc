= Installing BAMOE Task and Management Console
include::../partials/attributes.adoc[]

== Using Helm charts
BAMOE Consoles is a Helm chart based on the open-source KIE Runtime Tools Consoles Helm Chart that can be used to deploy BAMOE Task and Management Console images on a Kubernetes cluster using the Helm package manager.
...
First, you may need to get the default Kubernetes cluster domain for your ingress with this command:
