...
With {PRODUCT_LONG} version 9.1 aligning with its previous version {PRODUCT_SHORT} version 8.0 in the downstream build model, the artifacts are no longer available in Maven Central.
...
* bamoe-{VERSION}-maven-repository-image.tar.gz
* the hosted quay.io/bamoe/maven-repository:{IMAGE_VERSION} image
...
. Download the {PRODUCT_SHORT} version {VERSION_911} Maven Repository (`bamoe-{VERSION}-maven-repository.zip`).
