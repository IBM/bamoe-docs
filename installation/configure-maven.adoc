...
[source,shell,subs="attributes+"]
----
docker run -d -p <PORT>:8080 quay.io/bamoe/maven-repository:{IMAGE_VERSION_910}
...
. Download the dedicated M2 repository image container (`bamoe-9.1.x-maven-repository-image.tar.gz`).
...
. Download the {PRODUCT_SHORT} v{VERSION_910} Maven Repository (`bamoe-9.1.x-maven-repository.zip`).
...
. Add the following lines to the `<profiles>` element of the Maven settings.xml file, where `<MAVEN_REPOSITORY>` is the path of the Maven repository that you downloaded.
  The format of `<MAVEN_REPOSITORY>` must be `file://$PATH`. For example, `file:///home/userX/bamoe-9.1.x-maven-repository/:`.
+
[source]
----
<profile>
