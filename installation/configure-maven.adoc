= Installing BAMOE Maven repository
...
Instead the Maven libraries are shipped in the Maven repository format as a .tar.gz file or via HTTP through a Container image you can deploy to your infrastructure.
...
For organizations that already use a Maven Repository Manager tool, developers and the CI systems are usually configured to connect to it.
