...
KIE Sandbox and its related services can be run on a single machine using `docker-compose`.
...
Run KIE Sandbox with docker compose and current environment variables using pnpm:
...
KIE Sandbox will be available at http://localhost:9090.
