...
These Helm charts include the necessary licensing annotations and do not require any manual configuration as was the case in previous releases. Required annotations are injected into all deployed containers. This ensures accurate license tracking from deployment, and simplifies the installation process. The charts are pre-configured with the correct product IDs, product names, and metrics for each (PRODUCT_SHORT) offering.
...
The recommended way of installing Canvas and its backend applications is through the Dev environment Helm chart you are licensed to install. 

{CANVAS} and its two backend applications are also available as container images which are published to `Quay.io` and can be used to run containers locally or with any cloud provider.
...
== Maven Repository deliverables
The Maven libraries required to develop _Business Services_ with {PRODUCT_SHORT} are shipped as a separate container and are part of the Dev environment Helm chart.

{MVN_REPO}:: https://quay.io/repository/bamoe/maven-repository[quay.io/bamoe/maven-repository:{IMAGE_VERSION}]
...
The Helm chart provides a simple way to deploy {CANVAS} and {MVN_REPO} in Kubernetes, OpenShift, and Minikube with pre-configured licensing depending on your licensed product (see xref:licensing.adoc[License Guide]).
...
To install {CANVAS} on OpenShift, first get the default OpenShift domain for your routes using the following command:
...
[source,shell,subs="attributes+"]
----
helm pull oci://quay.io/bamoe/dmoe-dev-environment-helm-chart --version=9.4.0 --untar
...
Where you should replace `dmoe-dev-environment-helm-chart` with `pamoe-dev-environment-helm-chart` if you are working with {PRODUCT_PAM} and <YOUR_OCP_ROUTE_DOMAIN> with the domain of your OpenShift Route.
...
Where you should replace `dmoe-dev-environment-helm-chart` with `pamoe-dev-environment-helm-chart` if you are working with {PRODUCT_PAM} and <YOUR_KUBERNETES_CLUSTER_DOMAIN>` with the domain of your cluster and `<YOUR_KUBERNETES_INGRESS_CLASS>` with the ingress class of your cluster.
...
Where you should replace `dmoe-dev-environment-helm-chart` with `pamoe-dev-environment-helm-chart` if you are working with {PRODUCT_PAM}. 
