...
The recommended way of installing {MVN_REPO} and {CANVAS} is to install the pre-configured Helm charts for the {PRODUCT_SHORT} Offering that you have purchased licenses for, {PRODUCT_DM} or {PRODUCT_PAM}:
...
== Deploy {CANVAS} in an air-gapped OpenShift environment

If you have an air-gapped OpenShift cluster and need to deploy {CANVAS} using images from the internal registry, follow these steps. This procedure involves pulling images from Quay.io, pushing them to the OpenShift internal registry, and deploying the application.
