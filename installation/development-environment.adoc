...
{DEV_TOOLS}:: to enable efficient development and collaboration on _Business Services_ within an industry standard IDE.
...
For more information on the product distribution see xref:../release-notes/whats-new-9-5.adoc#download-doc[What's New → Download Documentation]. 
...
The recommended way of installing {MVN_REPO} and {CANVAS} is to install the pre-configured Helm charts for the {PRODUCT_SHORT} offering that corresponds to your license:

* pamoe-{VERSION}-dev-environment-helm-chart for {PRODUCT_PAM}.
* dmoe-{VERSION}-dev-environment-helm-chart for {PRODUCT_DM}.

These Helm charts include the necessary licensing annotations, avoiding the need for the manual configuration required in previous releases. The annotations are injected into all deployed containers and ensure accurate license tracking from deployment. The charts are pre-configured with the correct product IDs, product names, and metrics for each {PRODUCT_SHORT} offering, greatly simplifying the installation process.

The charts enable the configuration of Maven, Canvas and its backend services through Helm parameters which are automatically converted to environment variables during deployment. This enables the centralization of all configuration parameters, in a single `values.yaml` that can be managed in version control systems, see xref:#helm-chart-config-ref [Configuration reference for Dev Helm Charts]

Each Dev Helm chart contains four sub-charts which you can also install individually. However if you do choose this option you will need to manage licensing and other configuration manually:

* canvas: The main {CANVAS} application for modeling BPMN and DMN, available via https://quay.io/repository/bamoe/canvas[quay.io/bamoe/canvas:{IMAGE_VERSION}]
* cors_proxy: The service for cross-origin requests, that enables {CANVAS} to communicate directly from the browser, with Git and Cloud providers, available via https://quay.io/repository/bamoe/cors-proxy[quay.io/bamoe/cors-proxy:{IMAGE_VERSION}]
* extended_services: The service that enhances {CANVAS} capabilities with the DMN Runner and BPMN and DMN model static validation services, available via https://quay.io/repository/bamoe/extended-services[quay.io/bamoe/extended-services:{IMAGE_VERSION}]
* maven_repository: The Maven artifact repository required to develop _Business Services_, available via https://quay.io/repository/bamoe/maven-repository[quay.io/bamoe/maven-repository:{IMAGE_VERSION}]

All sub-charts receive License Service annotations automatically through the global configuration of the parent chart. The annotations are applied to every pod deployment,
...
* Linux/x86_64 architecture which runs on other platform architectures supported by {PRODUCT_SHORT}.

If you prefer to install the Dev environment components individually using OpenShift CLI and Docker this can be done using the individual {PRODUCT_SHORT} container images. They are available as multi-architecture images on Quay.io. The host machine architecture is automatically resolved when you pull the container images. You can use the following command to specify a platform if required:
...
Where `dmoe-dev-environment-helm-chart` is replaced by `pamoe-dev-environment-helm-chart` if you are working with {PRODUCT_PAM}.
...
Where `dmoe-dev-environment-helm-chart` is replaced by `pamoe-dev-environment-helm-chart` if you are working with {PRODUCT_PAM}.`YOUR_KUBERNETES_CLUSTER_DOMAIN` and `YOUR_KUBERNETES_INGRESS_CLASS` are defined as the domain and the Ingress class of your cluster, respectively.

=== Minikube install

To install {CANVAS} Helm chart on a Minikube cluster, you will need:

- https://minikube.sigs.k8s.io/docs/start/[A local Minikube cluster up and running]
...
=== The {CANVAS} OpenShift topology
image::bamoe-canvas-openshift-topology.png[OpenShift topology after {CANVAS} deployment]

Your {CANVAS} instance is now up and accessible. To get the route run the following command:
...
The {MVN_REPO} is deployed and configured by default during the Dev environment installation. After installation it can be accessed at:
...
This chart uses default environment variables from `values.yaml` file. You can override the variables by passing them through the command line. See xref:#helm-chart-config-ref[Dev Helm Chart configuration reference] for all options.
...
If you have an OpenShift cluster and are logged in by using the `oc` CLI tool, follow these steps to deploy {CANVAS} and its dependencies. Note that you will need to add licensing annotations manually if you follow this procedure rather than using the Dev environment Helm chart. 
...
One of the important environment variable to consider when deploying {CANVAS} is `KIE_SANDBOX_DEV_DEPLOYMENT_BASE_IMAGE_URL`. It points to a container image that is used when creating a Dev Deployment.
...
For more information on deploying {CANVAS} in an air-gapped OpenShift environment, see xref:#air-gapped-canvas-deployment[Deploy {CANVAS} in an air-gapped OpenShift environment].
...
The `bamoe-{VERSION}-canvas-docker-compose.zip` can only be downloaded from IBM Passport Advantage, and you can find information on the https://www.ibm.com/support/pages/node/7277082[BAMOE {VERSION_RM} Download Document].
...
////
Not clear why this is necessary as it does not contain any additional information
...
[#air-gapped-canvas-deployment]
== Install {CANVAS} in an air-gapped OpenShift environment

If you are installing in an air-gapped OpenShift cluster and need to install the Dev Helm chart using images from the internal registry, you pull images from Quay.io, push them to the OpenShift internal registry, before deploying the environment components.
...
----
+
. Select or create the target Namespace where the {CANVAS} application will be deployed.
+
[source,shell]
----
...
----
+
. Pull the {CANVAS} images from the Quay repository.
+
[source,shell,subs="attributes+"]
----
podman pull --arch amd64 quay.io/bamoe/canvas:{IMAGE_VERSION}
podman pull --arch amd64 quay.io/bamoe/cors-proxy:{IMAGE_VERSION}
podman pull --arch amd64 quay.io/bamoe/extended-services:{IMAGE_VERSION}
----
+
...
[source,shell,subs="attributes+"]
----
podman tag quay.io/bamoe/canvas:{IMAGE_VERSION} $REGISTRY/$NAMESPACE/canvas:{IMAGE_VERSION}
podman tag quay.io/bamoe/cors-proxy:{IMAGE_VERSION} $REGISTRY/$NAMESPACE/cors-proxy:{IMAGE_VERSION}
podman tag quay.io/bamoe/extended-services:{IMAGE_VERSION} $REGISTRY/$NAMESPACE/extended-services:{IMAGE_VERSION}
----
+
...
[source,shell,subs="attributes+"]
----
podman push $REGISTRY/$NAMESPACE/canvas:{IMAGE_VERSION} --tls-verify=false
podman push $REGISTRY/$NAMESPACE/extended-services:{IMAGE_VERSION} --tls-verify=false
podman push $REGISTRY/$NAMESPACE/cors-proxy:{IMAGE_VERSION} --tls-verify=false
----
+
...
canvas              default-route-openshift-image-registry.apps.example.com/bamoecanvas/canvas                                9.3.1-ibm-0006   2 minutes ago
cors-proxy          default-route-openshift-image-registry.apps.example.com/bamoecanvas/cors-proxy                            9.3.1-ibm-0006   21 seconds ago
extended-services   default-route-openshift-image-registry.apps.example.com/bamoecanvas/extended-services                     9.3.1-ibm-0006   About a minute ago
----
+
...
[source,shell,subs="attributes+"]
----
oc new-app image-registry.openshift-image-registry.svc:5000/$NAMESPACE/extended-services:{IMAGE_VERSION} --name=$APP_NAME_EXTENDED_SERVICES
...
----
+
. Deploy the {CORS_PROXY} and label its resources.
+
[source,shell,subs="attributes+"]
----
oc new-app image-registry.openshift-image-registry.svc:5000/$NAMESPACE/cors-proxy:{IMAGE_VERSION} --name=$APP_NAME_CORS_PROXY
...
----
+
. Deploy the {CANVAS} application and configure environment variables to connect to the backend services.
+
[source,shell,subs="attributes+"]
----
oc new-app image-registry.openshift-image-registry.svc:5000/$NAMESPACE/canvas:{IMAGE_VERSION} --name=$APP_NAME_BAMOE_CANVAS \
  -e KIE_SANDBOX_EXTENDED_SERVICES_URL=https://$(oc get route $APP_NAME_EXTENDED_SERVICES --output jsonpath={.spec.host}) \
  -e KIE_SANDBOX_CORS_PROXY_URL=https://$(oc get route $APP_NAME_CORS_PROXY --output jsonpath={.spec.host})
...
bamoe-canvas              bamoe-canvas-bamoecanvas.apps.example.com                                       bamoe-canvas              8080-tcp    edge          None
bamoe-cors-proxy          bamoe-cors-proxy-bamoecanvas.apps.example.com                                   bamoe-cors-proxy          8080-tcp    edge          None
bamoe-extended-services   bamoe-extended-services-bamoecanvas.apps.example.com                            bamoe-extended-services   21345-tcp   edge          None
----

The {CANVAS} application is now accessible at the route URL shown in the output (for example, `\https://bamoe-canvas-bamoecanvas.apps.example.com`).


== Setting up the {MVN_REPO} locally

If you have a Maven Repository Manager such as Artifactory or Nexus, you can import the compressed {PRODUCT_SHORT} Maven repository content into Maven Repository Manager. Note that you will need to add licensing annotations manually if you follow this procedure rather than using the Dev environment Helm chart. 
...
. Open your local Maven `~/.m2/settings.xml` file in a text editor or IDE.
+
...
. Create the `~/.gradle/init.d/` directory, if it does not exist. Any Gradle build executed on this machine will automatically run any scripts placed in this directory during the initialization phase, allowing you to apply {PRODUCT_SHORT} configuration globally.
...
def bamoeRepoUrl = "http://localhost:9999/"
...
. Download the {MVN_REPO}, `bamoe-{VERSION}-maven-repository.zip`.
. Extract the downloaded archive.
...
{PRODUCT_SHORT} container images can optionally serve traffic directly over HTTPS using a TLS certificate and a key provided by the customer. This enables end-to-end encryption between the user browser and the container on OpenShift, through a passthrough Route. By default TLS not active and your existing deployment behavior is maintained. 
...
| Extended Services 
| `EXTENDED_SERVICES_TLS_CERTIFICATE_PATH` 
| `EXTENDED_SERVICES_TLS_KEY_PATH`  
|===
...
. In the Helm values file for the {PRODUCT_SHORT} component, enable TLS and reference the Secret:
+
[source,yaml]
----
tls:
...
. Run `helm install` (or `helm upgrade`) as usual.
...
For more information on how to implement this in the Runtime environment Helm chart containers see xref:runtime-environment.adoc#runtime-env-tls-cert[Configuring Runtime containers with Passthrough Transport Layer Security (TLS)]

[#canvas-gitrepo-config]
== Configuring {CANVAS} to work with supported Git providers

To integrate GitHub, GitHub Enterprise Server, GitLab, or Bitbucket Data Center with {Canvas}, you must configure the `KIE_SANDBOX_AUTH_PROVIDERS` environment variable using the `canvas.env` Helm configuration parameter (see xref:#helm-chart-config-ref[Configuration reference for Dev environment Helm Charts]). 
...
For more information, see xref:../tools/applying-accelerators.adoc#accessing-accelerators[Accessing Accelerators].
...
For more information on using accelerators, see xref:../tools/applying-accelerators.adoc[Using Accelerators]
...
Once you have successfully installed and configured {CANVAS} and {MVN_REPO} via the Dev environment Helm chart, as well as {DEV_TOOLS}, see the following sections:

* xref:../getting-started/project-setup.adoc[Getting Started → Business Service project setup] for more information on Business Service projects.
...
* xref:../tools/developer-tools-for-vscode.adoc[{DEV_TOOLS}] for more information on the main features available in {DEV_TOOLS}.
