...
|{CANVAS}
|A standalone web application that provides authoring and testing tools for Decisions and Processes in your browser designed primarily for Business Analysts.
|{PRODUCT_DM}, {PRODUCT_PAM}

|{DEV_TOOLS}
|An integration with Microsoft VS Code editor provides the tools to create, edit, test, and visualize business processes (BPMN), decisions (DMN) and test scenarios (SceSim). Combined with existing Java extensions, provide a fully integrated and consistent workspace for Business Service developers.
|{PRODUCT_DM}, {PRODUCT_PAM}
...
|{MVN_REPO}
|An image or a zip file which contains all the third-party libraries referenced by core BAMOE components, ensuring that component versions and references are maintained with supported versions.
|{PRODUCT_DM}, {PRODUCT_PAM}
...
|{QUARKUS_DEV_UI}
|Tools and views into a Business Service, that can be used during development, to interact and view _Business Services_. The {QUARKUS_DEV_UI} is built upon the existing Quarkus DevUI.
|{PRODUCT_DM}, {PRODUCT_PAM}
//validate console is also part of DM?

|{SPRING_WEB_CONSOLE}
|Tools and views into a _Business Service_, that can be used during development, to interact and view _Business Services_. The {SPRING_WEB_CONSOLE} is built via Spring Boot and existing assets from the {QUARKUS_DEV_UI}.
|{PRODUCT_DM}, {PRODUCT_PAM}
//validate console is also part of DM?

|{PRODUCT_SHORT} Data Cleanup Tool 
|A tool written in Java to remove out-of-date information from the production database, ensuring queries continue at acceptable levels of performance.
|{PRODUCT_PAM}
