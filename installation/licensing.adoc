...
License reporting is available with IBM License Metric Tool (ILMT) in a non-container environment and with IBM License Service (ILS) in a container environment. ILMT tracks which software programs are installed on particular physical hardware or in VMs entitled under VPCs. ILS collects and measures license usage of products deployed in a cloud environment, OpenShift of Kubernetes. Depending on your environment the tool or service provide reports that can be presented to IBM when compliance around entitlement capacity is requested, and you can also configure automatic upload via ILMT or ILS. 
