...
In this section you will find all the information needed to implement any of the supported customization scenarios for the {PRODUCT_SHORT} container images. Customization is made possible by overriding the base operating system images while maintaining full functionality and compatibility with future {PRODUCT_SHORT} releases. 
...
. **Select Your Images**: Determine which BAMOE images need customization (see xref:#dev-images[Dev environment images] and xref:#runtime-images[Runtime environment images])
...
== Common override scenario summary
...
Your organization may require all containers to run on a specific OS version (e.g., UBI 9.6 instead of 9.7) for security compliance or standardization. This scenario applies to the following container images:
...
[#canvas-console-maven-os]
=== Canvas, Management Console and Maven Repository images
...
The following table lists the placeholder values for the three container images that can use the template:
...
The following points are important to understand when customizing Extended Services or the MCP Server image: 
...
=== Extended Services and MCP Server
As these two container images are Java based, you can use the same process described in Scenario 1, see xref:#extser-mcpserver-os[Overriding operating system version → Extended Services and MCP Server], and change the Java version in the base image.
...
Changing your Node.js version requires rebuilding the Node.js runtime in your base image.

The Dockerfile structure in this case is the following:

[source,dockerfile]
----
ROM quay.io/bamoe/cors-proxy:main AS source
...
Use the common template from Scenario 1 (see xref:#canvas-console-maven-os[Overriding operating system version → Canvas, Management Console and Maven]), and add the following security update step after installing httpd:
...
=== Extended Services, MCP Server

Use the common template from Scenario 1 (see xref:#extser-mcpserver-os[Overriding operating system version → Ext Services and MCP Server]) and add the following security update step:
...
=== Cors Proxy
Use the common template from Scenario 1 (see xref:#cors-proxy-os[Overriding operating system version → Cors Proxy]), and add the following security update step after the FROM statement:
...
To achieve this you can use the same Dockerfile templates from Scenario 1 (see xref:#canvas-os[Overriding operating system version]), but add performance optimization flags and settings.

=== Best practices for performance optimization (all images)
The following practices apply to all container images:

. Minimize Layers by combine RUN commands to reduce image layers
+
[source,dockerfile]
----
# ❌ Multiple layers
...
Use Specific Package Versions: Avoid unnecessary updates
