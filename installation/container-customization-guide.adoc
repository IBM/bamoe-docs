...
Node-js based images run Node.js applications using the Node.js runtime environment. The {PRODUCT_SHORT} containers are: Cors-proxy and their main characteristics are that they:

* execute JavaScript code on the server side
* use Node.js runtime (bundled via NVM * Node Version Manager)
* typically serve REST APIs or proxy services
* are lightweight and have fast startup times
...
The following template includes the common operations for the only Node.js based image in {PRODUCT_SHORT}. It uses NVM (Node Version Manager) to manage the Node.js runtime:
...
Use this process for Docker Compose: 
