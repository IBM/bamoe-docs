...
== Introducing the {PRODUCT_SHORT} container images
...
Your organization may require all containers to run on a specific OS version (e.g., UBI 9.6 instead of 9.7) for security compliance or standardization. This scenario can apply to all container images:

[#httpd-os]
=== Common Dockerfile template for Httpd based images

Canvas, Management Console, and Maven Repository images all use `httpd` and follow a similar Dockerfile structure as shown in the following template. You can simply replace the placeholders with the appropriate values for your target image:
...
The following differences exist between the three images:
...
CORS Proxy is the only Node.js based image in {PRODUCT_SHORT}. It uses NVM (Node Version Manager) to manage Node.js runtime. The following template shows common operations:
...
Your organization may require a specific runtime version of a (e.g., Java 21 instead of Java 17, or Node.js 20 instead of 24). This scenario applies to the following container images:
...
. Rebuild the Node.js runtime in your base image.
...
To achieve this, you can use the same Dockerfile templates described in Scenario 1 (see xref:#override-os[Overriding operating system version]) and add security update commands to apply patches. 

=== Httpd based images
To override an httpd based container (e.g., Canvas, Management Console) add the following security update step to the common Dockerfile template, after installing httpd:
...
To override an Java based container (e.g., Extended Services, MCP Server) add the following security update step to the common Dockerfile template:
...
=== Node.js based images
To override a Node.js based container (e.g., Cors Proxy) add the following security update step to the common Dockerfile template after the FROM statement:
...
To achieve this you can use the same Dockerfile templates from Scenario 1 (see xref:#override-os[Overriding operating system version]) and add performance optimization flags and settings.
...
Add the following to the common dockerfile template to achieve your objective. 
...
=== Extended Services and MCP Server
Add the following to the common dockerfile template to replace the CMD with JVM performance tuning:
...
=== Cors Proxy
Add the following Node.js performance optimizations to the common dockerfile template:
...
This scenario applies to the following container images:
...
To achieve this you can use the same Dockerfile templates from Scenario 1 (see xref:#canvas-os[Overriding operating system version]), and add certificate installation and trust configuration steps.

=== Canvas, Management Console and Maven Repository
Add the following to the common dockerfile template: 
...
=== Extended Services and MCP Server
Add the following certificate integration steps to the common dockerfile template: 
