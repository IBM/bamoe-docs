...
== {PRODUCT_SHORT} container images

You can customize any of the containers delivered in the {PRODUCT_SHORT} distribution. The following table lists the container images that can be customized, along with their descriptions, helm charts, and types.
...
The {PRODUCT_SHORT} Java based images use OpenJDK base images. You can use the following template to customize your selected image:
...
=== Httpd based images
Use the common Dockerfile template for httpd based images (see xref:#httpd-template[Common Dockerfile template for httpd based images]). The following differences exist between the three main httpd images:
...
=== Node.js based images 
Use the common Dockerfile template for Node.js based images (see xref:#nodejs-template[Common Dockerfile template for NodeJS based images]). The following list describes the common operations for the CORS Proxy image:
...
[#java-os]
=== Java based images
Use the common Dockerfile template for Java based images (see xref:#java-template[Common Dockerfile template for Java based images]). 

The following points are important to understand when customizing Extended Services, MCP Server image or other Java based images: 
...
// check if I want to list later

=== Using a different Java version in Java based images
As Extended services and MCP Server container images are Java based, you can use the same process described in Scenario 1, see xref:#java-os[Overriding operating system version → Java based images], and change the Java version in the base image.
...
=== Httpd based images
To override an httpd based container (e.g., Canvas, Management Console) add the following security update step to the common Dockerfile template (see xref:#httpd-templates[Common Dockerfile templates for httpd based images]), after installing httpd:
...
=== Java based images
To override an Java based container (e.g., Extended Services, MCP Server) add the following security update step to the common Dockerfile template (see xref:#java-templates[Common Dockerfile templates for Java based images]):
...
=== Node.js based images
To override a Node.js based container (e.g., Cors Proxy) add the following security update step to the common Dockerfile template (see xref:#nodejs-templates[Common Dockerfile templates for Node.js based images]), after the FROM statement:
...
Add the following to the common Dockerfile template (see xref:#httpd-templates[Common Dockerfile template for httpd based images]) to achieve your objectives, for example to: 

* Optimize package installation.
* Combine permission commands into single layer.
* `--nodocs`: skip documentation to reduce image size
* `--setopt=install_weak_deps=0`: not install weak dependencies
...
. Edit httpd configurations to disable unnecessary or unused modules.
. Optimize Multi-Processing Module (MPM) Settings by tuning worker processes and threads based on workload
. Enable Compression and configure `mod_deflate` for response compression
...
=== Java based images
Add the options you require to the common dockerfile template (see xref:#java-templates[Common Dockerfile templates for Java based images])to replace the CMD with JVM performance tuning:
...
//remove docker file specifics

. The container aware JVM Settings are:
+
[source,dockerfile]
----
...
+
* G1GC is good for most workloads (`-XX:+UseG1GC`)
...
=== Httpd based images
Add the following to the common dockerfile template (see xref:#httpd-templates[Common Dockerfile template for httpd based images]):
...
=== Java based images
Add the following certificate integration steps to the common dockerfile template (see xref:#java-templates[Common Dockerfile template for JAva based images]):
...
- must be run with root privileges to modify truststore
...
=== Node.js based images
Add the following certificate integration steps to the common dockerfile template (see xref:#nodejs-templates[Common Dockerfile template for Node.js based images]): 
