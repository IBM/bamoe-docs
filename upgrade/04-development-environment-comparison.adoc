...
The transition from BAMOE version {VERSION_80X} to version {VERSION_92X} marks a significant shift in how business automation solutions are developed and maintained. While version {VERSION_80X} relied on a centralized, monolithic architecture centered around Business Central, version {VERSION_92X} introduces a modular, cloud-native approach that separates the responsibilities of business users and developers.

This section introduces the new development experience in BAMOE version {VERSION_92X}, highlighting the tools and architectural changes that support a more flexible and scalable workflow.
...
=== BAMOE version {VERSION_92X} - BAMOE Canvas
...
=== BAMOE version {VERSION_92X} - BAMOE Developer Tools for VS Code

image::upgrade/04-development-environment-bamoe-developer-tools.png[BAMOE Canvas]
...
Unlike Business Central, BAMOE version {VERSION_92X} does not use the concept of Spaces. Projects are now organized using Git repositories and follow standard Maven structures. A project can be implemented using Kogito with Quarkus, Spring Boot, or plain Java with Drools or jBPM.
...
In BAMOE version {VERSION_92X}, Git integration is a core part of the development workflow, but the approach differs between BAMOE Canvas and the BAMOE Developer Tools for VS Code.
...
In BAMOE version {VERSION_92X}, projects are built using standard Maven tools, and the build process is fully externalized. The BAMOE Maven Repository provides all required dependencies and can be deployed as a standalone container or installed locally.
...
In BAMOE version {VERSION_92X}, authentication is decentralized and aligned with modern cloud-native practices. Each tool--BAMOE Canvas and the BAMOE Developer Tools for VS Code--handles authentication differently, based on its integration with Git.
...
By shifting authentication to Git, BAMOE version {VERSION_92X} eliminates the need for centralized user management within the platform itself, offering a more flexible and secure model that aligns with modern development environments.
...
BAMOE version {VERSION_92X} adopts a more open and developer-friendly model. Projects are structured as standard Maven applications, allowing developers to build using the Maven CLI, integrate with IDEs, and automate deployments through CI/CD pipelines. Deployment targets include Kubernetes-native solutions.
...
The Standard Operating Environment (SOE) for BAMOE has evolved significantly between version {VERSION_80X} and version {VERSION_92X}. These changes reflect the shift toward a more cloud-native, containerized architecture in BAMOE version {VERSION_92X}, which impacts supported platforms, middleware, and system requirements.
