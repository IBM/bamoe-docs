...
You can deploy an BAMOE 8 project to a configured KIE Server, to an embedded Java application, or into a Red Hat OpenShift Container Platform environment for an enhanced containerized implementation. For more information on deployment options, see Project deployment options with IBM Business Automation Manager Open Editions.
...
a|
BAMOE 9 allows for creating domain-specific APIs that can be exposed through REST endpoints or events. BAMOE 9 does not support APIs to interact with containers and servers as business assets are no longer deployed in internal KIE Server.
