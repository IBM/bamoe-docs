...
----
If your project uses _io.quarkus:quarkus-oidc_, you may also need to add the following line:
...
Before you begin, make sure you upgrade both the services and the database together. Business Services running BAMOE 9.2.0 won’t work with a database that you’ve already migrated to BAMOE 9.2.1.

To upgrade safely, back up your PostgreSQL and MS SQL database, test the migration in a staging environment, apply the scripts sequentially for each subsystem, schedule downtime for production, and verify that all services function correctly after the upgrade.
