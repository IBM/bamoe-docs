...
PMML models can be referenced in DMN functions.
If that is the case of your Business Service project, please see also <<predictive-models-pmml,PMML section>>.

*Tooling Point of View*

If your DMN model references PMML functions, both the v8 and v9 editors are capable of loading PMML metadata into the DMN Function expression editor, provided the function kind is set to PMML. From a runtime perspective, your project must include the appropriate dependency to enable PMML execution.

*Engine Point of View*

Your project that combines DMN and PMML should contain this dependency:
...
BAMOE v9 does not support authoring or editing PMML models. You can continue to use your existing PMML assets from BAMOE v8, but only in read-only mode. This means that while PMML models can still be referenced and executed, they must be maintained externally using third-party tools.

=== Engine Point of View

In BAMOE v8, PMML execution was supported through the external module `org.kie:kie-dmn-jpmml`, which wrapped the `org.jpmml:jpmml-evaluator` engine. This engine supported a wide range of PMML model types. However, due to licensing restrictions, the `jpmml-evaluator` engine is no longer available in BAMOE v9, and the wrapper module has been removed.

BAMOE v9 introduces native support for a subset of PMML model types, specifically: Regression, Tree, Scorecard, Cluster, and Mining. To enable PMML execution in BAMOE v9, your project must include the `kogito-pmml` dependency with appropriate exclusions to avoid conflicts with Drools-based components.

Here is the recommended Maven configuration:
