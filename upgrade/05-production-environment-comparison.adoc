...
image::upgrade/v8-production-environment-bamoe.png[v8 arch]
...
The BAMOE Management Console provides a lightweight, stateless web interface for interacting with the runtime environment. It connects to the Data Index, Jobs Service, and User Tasks subsystems to retrieve and display information about process instances, user tasks, and jobs. The console can be deployed manually or using Helm charts, depending on the environment.
...
For more details, see Deploying to Kubernetes or OpenShift with the Helm Chart.
...
image::upgrade/v9-production-environment-bamoe.png[v9 arch]
...
Because Business Services are independent microservices, BAMOE v9 does not include a dedicated load balancer like the Smart Router. Instead, users are free to use any load balancing solution compatible with their chosen runtime. In OpenShift environments, Business Services can be deployed as pods and scaled automatically using OpenShift's native autoscaling capabilities.

BAMOE v9 does not provide its own OpenShift Operator. Instead, it offers Helm charts to simplify deployment. The BAMOE Managemnet Console Helm Chart is available for installing the related components on Kubernetes or OpenShift clusters. These charts support flexible configuration and align with infrastructure-as-code practices. For more details, see Deploying to Kubernetes or OpenShift with the Helm Chart.
...
| BAMOE v9 Business Service projects can be packaged as a Container image, WAR file, JAR file or Native Executable. They can also be packaged as kjar with pom.xml and kmodule.xml when using decision in Java embedded mode. See https://www.ibm.com/docs/en/ibamoe/9.2.x?topic=scenarios-executing-rules-in-embedded-java-mode[Executing Rules in embedded Java mode]
...
| Use https://www.ibm.com/docs/en/ibamoe/9.2.x?topic=workflows-process-api[Process API], https://www.ibm.com/docs/en/ibamoe/9.2.x?topic=workflows-user-tasks-api[User Tasks API], https://www.ibm.com/docs/en/ibamoe/9.2.x?topic=workflows-data-index-graphql-api[Data-Index API]
...
| Prometheus extension can be enabled by setting `org.kie.prometheus.server.ext.disabled` to false when starting process server. If using BAMOE v8 on springboot, use `kieserver.prometheus.enabled=true` to enable prometheus.
...
Production Standard Operating Environment (SOE) differences between BAMOE v8 and v9 remains same as Development one's except with minor differences.
...
| Java 11
| Java 17 (recommended for enhanced performance), GraalVM JDK 17 (For native builds)

| *Operating Systems (OS)*
| Windows, Linux
| Microsoft Windows Server 2022 , Red Hat Enterprise Linux 9.5 (RHEL), macOS Sequoia

| *Container platform*
| OpenShift Container Platform 4.16, Kubernetes 1.30
| OpenShift Container Platform 4.16, 4.17 and 4.18 , Kubernetes 1.30

| *Maven Version*
| Maven 3.6.2+ (minimum) - Commonly used with Maven 3.8.6
| Maven 3.9.6 (required for examples and builds) - Backward compatible with 3.8.x

| *Databases*
| PostgreSQL - MySQL - Oracle - Microsoft SQL Server (via KIE Server)
| PostgreSQL 17.4 , Microsoft SQL Server (2022)
...
https://www.ibm.com/docs/en/ibamoe/9.2.x?topic=scenarios-authoring-unit-tests-test[Test Scenarios] supported in BAMOE Developer Tools for VS Code(New Test Scenario Editor compatible with DMN 1.5)
|===
