...
. compile the application with `mvn clean install` (or similar, provided the `test` phase is invoked). If you are using Gradle, use the `gradle clean build` command.
