...
BAMOE v8 forms are not compatible with the {PRODUCT_SHORT} v9 form infrastructure. As a result, all forms must be recreated from scratch when migrating to BAMOE v9. This is due to significant changes in the underlying form model and rendering engine. 

In BAMOE v9, form generation is done in {DEV_TOOLS}. {CANVAS} does not support form creation or editing. For more information, see xref:../tools/form-generation.adoc[Form Code Generation].
