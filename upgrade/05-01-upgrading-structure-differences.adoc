...
| Optional; can be declared in code using `KieModuleModel` API or annotations. Check https://kie.apache.org/docs/10.0.x/drools/drools/KIE/index.html#_definingakiemoduleprogrammatically[Drools documentation].
...
| BAMOE supports custom persistence via Quarkus or Spring Boot configuration mechanisms. Read more on https://ibmdocs-test.dcs.ibm.com/docs/en/ibamoe/9.2.x?topic=workflows-configuring-persistence-stateful[Configuring persistence for stateful Workflows]

| *Test Scenario*
| `.scesim` files are placed under `src/test/resources` +
JUnit 4 support +
Activator Annotation -- `@RunWith(org.drools.workbench.screens.scenariosimulation.
backend.server.runner.ScenarioJunitActivator.class)`
| `.scesim` files are placed under `src/test/resources` +
JUnit 5 compatible +
