...
| Claim Tasks +
Users can directly submit a task if it is assigned to the current user or user group. They can also use xref:../workflow/user-tasks.adoc#taskid-transition2[BAMOE User Task API] to claim specific task. If authenticated as a user with xref:../rmanaging-monitoring/consoles.adoc#impersonation[impersonation] powers they can also submit on behalf of other users.
