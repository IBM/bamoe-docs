...
| Claim Tasks +
Users can directly submit a task if it is assigned to the current user or user group. They can also use xref:../workflow/user-tasks.adoc#taskid-transition2[BAMOE User Task API] to claim specific task. If authenticated as a user with xref:../runtime-services-modeling/consoles.adoc#impersonation[impersonation] powers they can also submit on behalf of other users.
...
| Set task priority and due date +
BAMOE Management Console does not support this feature. Use xref:../workflow/management-apis.adoc#management-api[Process Management API] instead.
...
| BAMOE v9 rely on Quarkus or Spring Boot security for authentication and authorization. see xref:../runtime-services-modeling/security.adoc[Securing Business Service API] endpoints.
...
| Users can use Quarkus or Spring Boot API authorization mechanisms to allow only users with specific roles to access specific endpoints. See xref:../runtime-services-modeling/security.adoc##authorization-quarkus[Authorization]
...
| Use xref:../workflow/management-apis.adoc[Process Management API], xref:../workflow/process-api.adoc[Process API], xref:../workflow/user-tasks.adoc[User Tasks API], xref:../workflow/data-index-graphql-api.adoc[Data-Index GraphQL API], and xref:../workflow/data-audit-api.adoc[Data-Audit API].
...
| Supports default and custom Graphana dashboard. See xref:../runtime-services-modeling/monitoring-services.adoc#grafana[Custom Grafana dashboards in IBM BAMOE].
...
| Follow the same steps as BAMOE v8 openshift deployment but with changes in application properties to enable/disable metrics and changes in the endpoints. See xref:../runtime-services-modeling/monitoring-services.adoc#config-prometheus[Configuring Prometheus].
|===
